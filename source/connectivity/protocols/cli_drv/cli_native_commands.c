...
bool cli_help_command(int32_t argc, char* argv[]) {
    ...
    uint32_t cnt = cli_get_command_cnt();
    cli_printf("CmdCnt:%u" CRLF, cnt);
    if(3 <= argc) {
...
bool cmd_echo(int32_t argc, char* argv[]) {
    ...
    uint8_t num = 1;
    if(0 == argc) {
        res = cli_toggle_echo(num);
    }

    if(1 == argc) {
        res = try_str2bool(argv[0], &echo);
        if(false == res) {
            LOG_ERROR(SYS, "ParseErr status %s", argv[0]);
        }
        res = cli_set_echo(num, echo);
        if(res) {
            LOG_INFO(SYS, "OK");
        }
    } else {
...
bool cli_diag_command(int32_t argc, char* argv[]) {
    ...
    cli_printf(TABLE_LEFT "%15s " TABLE_SEPARATOR "%16" PRId64 " " TABLE_SEPARATOR "%12" PRId64 " " TABLE_SEPARATOR
              ...
              "cmd_reader", CliInstance[0].Reader.tx_cnt,
			  CliInstance[0].Reader.lost_char_count,
			  CliInstance[0].Reader.total_string_count,
              CliInstance[0].Reader.error_count);
#endif
