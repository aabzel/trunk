...
const uint8_t gsc_ascii_1206[95][12] =
{
...
const uint8_t gsc_ascii_1608[95][16] =
{      
...
const uint8_t gsc_ascii_2412[95][36] =
{      
...
const FontInfo_t FontInfo[] = {
        {.size = 12, .font=FONT_12, .name="12x6", .dimension={.dx=6,.dy=12,}, .byte_in_row=2, },
        {.size = 16, .font=FONT_16, .name="16x8", .dimension={.dx=8,.dy=16,},.byte_in_row=2, },
        {.size = 36, .font=FONT_24, .name="24x12", .dimension={.dx=12,.dy=24,}, .byte_in_row=3,},
};


const FontInfo_t* Font2FontInfo(Font_t font){
    const FontInfo_t* Node = NULL;
    uint32_t cnt = ARRAY_SIZE(FontInfo);
    uint32_t i = 0;
    for(i=0; i<cnt; i++) {
        if(font == FontInfo[i].font){
            Node = &FontInfo[i];
            break;
        }
    }
...
FontFrame_t Font2FontDimension(Font_t font) {
    FontFrame_t dimension = {0};
    const FontInfo_t* Node = Font2FontInfo(font);
    if(Node ) {
        dimension=Node->dimension;
    } else {
        LOG_ERROR(FONT,"LackOfFont %u", font);
    }
    return dimension;
}
...
int8_t Font2ByteInRow(Font_t font) {
    int8_t byte_in_row = 0;
    const FontInfo_t* Node = Font2FontInfo(font);
    if (Node) {
        byte_in_row = Node->byte_in_row;
    } else {
        LOG_ERROR(FONT,"LackOfFont %u", font);
    }
    return byte_in_row;
...
#ifdef HAS_FONT_16_X_8
        case FONT_16: {
            memcpy(&font_word, &gsc_ascii_1608[chr_ind][ x*bytes_in_row],bytes_in_row);
...
#ifdef HAS_FONT_24_X_12
        case FONT_24: {
            memcpy(&font_word32, &gsc_ascii_2412[chr_ind][ x*bytes_in_row],bytes_in_row);

        }break;
#endif

        default:
            LOG_ERROR(FONT,"UndefFont %u", font);
            break;
    }

    uint8_t msb_idx = bytes_in_row*8-1;

    switch((uint8_t)font){
        case FONT_12:
        case FONT_16:{
            font_word = reverse_byte_order_uint16(font_word);
#ifdef HAS_BIN_2_STR
            LOG_PARN(FONT,"%s"CRLF,utoa_bin16_plain(  font_word));
#endif
            pixel= GET_BIT_NUM(font_word, msb_idx-y);
        }break;
        case FONT_24:{
            font_word32 = reverse_byte_order_uint24(font_word32);
#ifdef HAS_BIN_2_STR
            LOG_PARN(FONT,"%s"CRLF, utoa_bin24_plain(  font_word32));
#endif
