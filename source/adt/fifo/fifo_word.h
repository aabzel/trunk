...
uint32_t fifo_word_get_count(FifoWord_t* const instance);
uint32_t fifo_word_get_size(FifoWord_t* const instance);
uint32_t fifo_word_get_spare(FifoWord_t* const instance);

bool fifo_word_clean(FifoWord_t* instance);
bool fifo_word_free(FifoWord_t* fifo, uint32_t size);
bool fifo_word_reset(FifoWord_t* const instance);
bool fifo_word_init(FifoWord_t* const instance, int16_t* const inArray, uint32_t capacity);
