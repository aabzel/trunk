...
char* NumberParserFsm2Str(const Text2NumberFsm_t* const Node) {
    ...
    sprintf(name, "[%s] State:%s, ManSign %s Int:%Le Frac:%f ,Order %u ExpSign:%s ExpInt %u Value:%f",
            SymbolToStr(Node->cur_letter), NumberParserState2Str(Node->state), Sign2Str(Node->sign), Node->integer,
            Node->fraction, Node->fraction_order, Sign2Str(Node->exponent_sign), Node->exponent_integer, Node->value);
    return name;
}
