...
bool array_is_exist(const char symbol, const char* const addr, const uint32_t len);
bool uniq_array(const uint8_t* const addr, uint32_t len);
bool is_arr_pat2(const uint8_t* const data, const uint32_t size, const uint8_t patt, uint32_t * constdiff_index) ;
bool is_arr_pat(const uint8_t* const arr, const uint32_t size, const uint8_t patt);
