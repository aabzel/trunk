...
bool test_iir_proc(void) {
    LOG_INFO(IIR, "%s():", __FUNCTION__);
    bool res = true;
    set_log_level(IIR, LOG_LEVEL_DEBUG);
    uint32_t i = 0;
    IirSample_t x[TEXT_SIGNAL_LEN];
    IirSample_t y[TEXT_SIGNAL_LEN];

    for(i = 0; i < TEXT_SIGNAL_LEN; i++) {
        x[i] = calc_pwm_sample(SAMPLE_PERIOD_US * i, TEST_FREQ_HZ, 0.0, 10, TEST_DUTY_CYCLE, 0);
        ASSERT_TRUE(iir_proc_in_out(1, x[i], &y[i]));
    }
#ifdef HAS_PC
    ...
    set_log_level(IIR, LOG_LEVEL_INFO);
    return res;
}
...
static bool test_iir_performance_one(uint8_t num, uint32_t sample_rate_hz, uint32_t sample_cnt) {
    bool res = true;
    cli_printf(CRLF);
    LOG_INFO(IIR, "%s():IIR:%u,SamFreq:%u Hz,Sam:%u", __FUNCTION__, num, sample_rate_hz, sample_cnt);
    ...
    for(i = 0; i < sample_cnt; i++) {
        ASSERT_TRUE(iir_proc_in_out(num, x, &y));
    }
    ...
    LOG_INFO(IIR, "duratuin:%f us", total_duratuin_us);
    LOG_INFO(IIR, "SampleTime:%f s", sample_time_s);
    LOG_INFO(IIR, "SamplePtocAllowTime:%llu us", allowable_sample_time_us);
    LOG_INFO(IIR, "SamplePtocRealTime:%f us", real_sample_total_duratuin_us);
...
bool test_iir_performance(void) {
    ...
    ASSERT_TRUE(test_iir_performance_one(1, 48000, 4096));
    ASSERT_TRUE(test_iir_performance_one(2, 48000, 4096));
