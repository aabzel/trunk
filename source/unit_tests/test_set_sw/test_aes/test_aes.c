...
const uint8_t key_aes[] = {0x60, 0x3d, 0xeb, 0x10, 0x15, 0xca, 0x71, 0xbe, 0x2b, 0x73, 0xae,
                           0xf0, 0x85, 0x7d, 0x77, 0x81, 0x1f, 0x35, 0x2c, 0x07, 0x3b, 0x61,
                           0x08, 0xd7, 0x2d, 0x98, 0x10, 0xa3, 0x09, 0x14, 0xdf, 0xf4};
const uint8_t in_enc[16] = {0x6b, 0xc1, 0xbe, 0xe2, 0x2e, 0x40, 0x9f, 0x96,
                            0xe9, 0x3d, 0x7e, 0x11, 0x73, 0x93, 0x17, 0x2a};
const uint8_t exp_out_enc[] = {0xf3, 0xee, 0xd1, 0xbd, 0xb5, 0xd2, 0xa0, 0x3c,
                               0x06, 0x4b, 0x5a, 0x7e, 0x3d, 0xb1, 0x81, 0xf8};

const uint8_t in_dec[] = {0xf3, 0xee, 0xd1, 0xbd, 0xb5, 0xd2, 0xa0, 0x3c,
                          0x06, 0x4b, 0x5a, 0x7e, 0x3d, 0xb1, 0x81, 0xf8};

const uint8_t exp_out_dec[] = {0x6b, 0xc1, 0xbe, 0xe2, 0x2e, 0x40, 0x9f, 0x96,
                               0xe9, 0x3d, 0x7e, 0x11, 0x73, 0x93, 0x17, 0x2a};
...
bool test_decrypt_aes256_ecb(void) {
    ...
    memcpy(encrypted_data, in_dec, sizeof(in_dec));

    LOG_INFO(TEST, "CipherText");
    print_bin(encrypted_data, sizeof(encrypted_data), 0);

    LOG_INFO(TEST, "key_aes");
    print_bin(key_aes, sizeof(key_aes), 0);

#ifndef HAS_AES256_V1
    ASSERT_TRUE(aes256_ecb_decrypt(key_aes, encrypted_data, 16, out, &returnlen));
#else
    AES256MainECB(key_aes, in_dec, 16, out, &returnlen, false);
#endif
    ASSERT_EQ(16, returnlen);

    LOG_INFO(TEST, "PlainText");
    print_bin(out, sizeof(out), 0);

    ASSERT_TRUE(array_is_equal(out, exp_out_dec, 16));

    EXPECT_EQ_MEM(out, exp_out_dec, 16);
...
bool test_encrypt_aes256_ecb(void) {
    ...
    memcpy(plain_data, in_enc, sizeof(in_enc));
    ...
    uint32_t returnlen = 0;
#ifdef HAS_AES256_V1
    AES256MainECB(key_aes, in_enc, 16, out, &returnlen, true);
#else
    ASSERT_TRUE(aes256_ecb_encrypt(key_aes, plain_data, 16, out, &returnlen));
#endif
    ASSERT_EQ(16, returnlen);
    ...
    ASSERT_TRUE(array_is_equal(out, exp_out_enc, 16));
    EXPECT_EQ_MEM(out, exp_out_enc, 16);
