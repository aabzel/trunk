...
static uint8_t array_rx[I2S_TEST_BLOCK_SIZE] = { 0 };
static uint8_t array_tx[I2S_TEST_BLOCK_SIZE] = { 0 };
static uint16_t array_u16_tx[I2S_TEST_BLOCK_SIZE] = { 0 };
...
bool test_i2s_read_one(uint8_t num) {
    ...
    for (i = 0; i < 2; i++) {
#ifdef HAS_NRFX_I2S
        Node->i2s_buffers[i].p_rx_buffer = (uint32_t* ) array_rx;
        Node->i2s_buffers[i].p_tx_buffer = (uint32_t* ) array_tx;
#endif
    }

    ASSERT_TRUE(i2s_mcal_start(num, I2S_TEST_BLOCK_SIZE / 4));
...
bool test_i2s_write_single(uint8_t num) {
    ...
    I2sHandle_t *Node = I2sGetNode(num);
    ASSERT_NE(NULL, Node);
    Node->stop_needed = true;
    Node->total_stop_cnt = 0;
    Node->status_stop_cnt = 0;
    ASSERT_TRUE(wait_in_loop_ms(100));
    ...
    ASSERT_TRUE(i2s_mcal_write(num, (uint16_t* )array_tx, frames));
    ASSERT_TRUE(wait_in_loop_ms(100));

    EXPECT_GR(3, Node->total_stop_cnt); /*Still uncertainly why 3*/
    EXPECT_GR(1, Node->status_stop_cnt);
...
bool test_i2s_write_toggle(uint8_t num) {
    ...
    ASSERT_TRUE(i2s_mcal_write(num, (uint16_t* ) array1, 4));
    /*See Data change on the logic analyzer*/
    ASSERT_TRUE(wait_in_loop_ms(1000));
    ASSERT_TRUE(i2s_dma_stop(num));
    ASSERT_GR(0, Node->toggle_cnt);
...
bool test_i2s_loopback_single(uint8_t num) {
    ...
    Node->total_stop_cnt = 0;
#ifdef HAS_NRFX_I2S
    Node->i2s_buffers[0].p_rx_buffer = RxBuff;
    Node->i2s_buffers[0].p_tx_buffer = TxBuff;
    Node->i2s_buffers[1].p_rx_buffer = RxBuff;
    Node->i2s_buffers[1].p_tx_buffer = TxBuff;
#endif

    ASSERT_TRUE(i2s_mcal_start(num, 16));

    ASSERT_TRUE(wait_in_loop_ms(1000));
    ASSERT_GR(0, Node->it_cnt);
    EXPECT_GR(1, Node->total_stop_cnt);
...
bool test_i2s_loopback_continuous(uint8_t num) {
    ...
    nrfx_err_t ret = nrfx_i2s_next_buffers_set((nrfx_i2s_buffers_t const *) &(Node->i2s_buffers[1]));
...
bool test_i2s_write_one_qword(uint8_t num) {
    ...
    print_hex_buf(array_tx, I2S_TEST_BLOCK_SIZE);
    LOG_INFO(I2S, "Sample[0] 0x%08x", *((uint32_t*) &array_tx[0]));
    ASSERT_TRUE(i2s_mcal_write(num,( uint16_t* ) array_tx, I2S_TEST_BLOCK_SIZE / 4));
...
bool test_i2s_write_one_word(uint8_t num) {
    ...
    LOG_INFO(I2S, "InMemory");
    print_hex_buf((uint8_t*) array_u16_tx, 2);
    LOG_INFO(I2S, "Sample[0] 0x%04x", array_u16_tx[0]);
    ASSERT_TRUE(i2s_mcal_write(num, ( uint16_t* )array_u16_tx, ARRAY_SIZE(array_u16_tx)));
    ASSERT_TRUE(i2s_send(num, true));
    /*See Data change on the logic analyzer*/
    return true;
}
...
bool test_i2s_play_freq_com(uint8_t num, float freq, SampleType_t amp) {
    ...
    bool res = true;
    uint8_t dds_num = i2s_num_to_dds_num(num);
    uint32_t fps = 0;
    ASSERT_GR(0.0, freq);
    ASSERT_GR(0, amp);
    ...
    Node->switch_needed = false;
#ifdef HAS_DDS
    ASSERT_TRUE(dds_set_sin(dds_num, freq, amp, 0, (SampleType_t)0));
    ASSERT_TRUE(dds_set_array(dds_num, 1, 0));
    log_level_t ll = log_level_get(DDS);
    if(LOG_LEVEL_DEBUG==ll){
        dds_diag();
        ASSERT_TRUE(dds_print_track(TEST_DAC_NUM));
    }
#else
    res = false;
#endif /*HAS_DDS*/

    ASSERT_TRUE(dds_play_con(dds_num));
    return res;
}
...
bool test_i2s_play_freq(uint8_t num, float freq, SampleType_t amp, uint32_t pause_ms) {
    ...
    I2sHandle_t *Node = I2sGetNode(num);
    ASSERT_NE(NULL, Node);
    uint32_t orig_it_cnt = Node->it_cnt;
    ...
    LOG_INFO(I2S, "ItCnt: %d", Node->it_cnt - orig_it_cnt);
    ASSERT_GR(orig_it_cnt, Node->it_cnt);
    return true;
}
...
bool test_i2s1_read(void) {
    LOG_INFO(I2S, "%s()", __FUNCTION__);
    ASSERT_FALSE(i2s_mcal_read(1, array_rx, I2S_TEST_BLOCK_SIZE));
    return true;
}
#endif /*HAS_I2S1*/
...
bool test_i2s2_read(void) {
    LOG_INFO(I2S, "%s()", __FUNCTION__);
    ASSERT_FALSE(i2s_mcal_read(2, (uint16_t*) array_rx, I2S_TEST_BLOCK_SIZE));
    return true;
}
#endif /*HAS_I2S2*/
