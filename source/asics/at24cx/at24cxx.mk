...
ifneq ($(AT24CXX_MK_INC),Y)
    ...
    MCAL_OPT += -DHAS_AT24C02

    AT24CXX=Y
    I2C=Y
    MCAL_OPT += -DHAS_AT24CXX
    MCAL_OPT += -DHAS_AT24CXX_PROC
    ...
    ifeq ($(AT24CXX_WRITE),Y)
        ...
        MCAL_OPT += -DHAS_AT24CXX_WRITE
        SOURCES_C += $(AT24CXX_DIR)/at24cxx_write_drv.c
    endif

    ifeq ($(DIAG),Y)
        ifeq ($(AT24CXX_DIAG),Y)
            MCAL_OPT += -DHAS_AT24CXX_DIAG
    ...
    ifeq ($(CLI),Y)
        ifeq ($(AT24CXX_COMMANDS),Y)
            ...
            MCAL_OPT += -DHAS_AT24CXX_COMMANDS
