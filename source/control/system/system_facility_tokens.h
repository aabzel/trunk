#define ALL_FACILITY_INFO            \
ANALOG_FILTER_FACILITY_DIAG          \
ARRAY_FACILITY_DIAG                  \
AUTO_BRIGHTNESS_FACILITY_DIAG        \
AUTO_EXIT_FACILITY_DIAG              \
AUTO_VERSION_FACILITY_DIAG           \
BLACK_BOX_FACILITY_DIAG              \
BLE_AUDIO_FACILITY_DIAG              \
BOARD_FACILITY_DIAG                  \
BOOTLOADER_FACILITY_DIAG             \
BPSK_FACILITY_DIAG                   \
BUZZER_FACILITY_DIAG                 \
CALCULATOR_FACILITY_DIAG             \
SEGGER_RTT_FACILITY_DIAG             \
CALIBRATION_DATA_FACILITY_DIAG       \
CAN_DIFF_FACILITY_DIAG               \
CAN_RX_HIST_FACILITY_DIAG            \
DWT_FACILITY_DIAG                    \
CAN_TX_PLANNER_FACILITY_DIAG         \
CLOCK_DIVIDER_FACILITY_DIAG          \
CLOCK_FACILITY_DIAG                  \
CODE_STYLE_CHECKER_FACILITY_DIAG     \
COMPLEX_FACILITY_DIAG                \
CORE_FACILITY_DIAG                   \
CORRELATOR_FACILITY_DIAG             \
C_GENERATOR_FACILITY_DIAG            \
DDS_FACILITY_DIAG                    \
DECAWAVE_FACILITY_DIAG               \
DECAWAVE_NATIVE_FACILITY_DIAG        \
DECIMATOR_FACILITY_DIAG              \
DELTA_SIGMA_FACILITY_DIAG            \
DEMAGNETIZER_FACILITY_DIAG           \
DISK_IO_FACILITY_DIAG                \
DMA_CHANNEL_FACILITY_DIAG            \
DMA_FACILITY_DIAG                    \
DRV8870_FACILITY_DIAG                \
DSP_FACILITY_DIAG                    \
EEPROM_FACILITY_DIAG                 \
EIM_FACILITY_DIAG                    \
END_OF_BLOCK_FACILITY_DIAG           \
ERM_FACILITY_DIAG                    \
ESP_01_FACILITY_DIAG                 \
EXT_INT_FACILITY_DIAG                \
FCSMU_FACILITY_DIAG                  \
FFT_FACILITY_DIAG                    \
FILE_API_FACILITY_DIAG               \
FILE_PC_FACILITY_DIAG                \
FW_LOADER_FACILITY_DIAG              \
GAME_PAD_PS2_FACILITY_DIAG           \
GARLAND_FACILITY_DIAG                \
GM67_FACILITY_DIAG                   \
GPS_FACILITY_DIAG                    \
GRAPHVIZ_TO_TSORT_FACILITY_DIAG      \
HASH_TABLE_FACILITY_DIAG             \
HEX_BIN_FACILITY_DIAG                \
HIST_FILTER_FACILITY_DIAG            \
HW_CRYP_FACILITY_DIAG                \
HW_VERSION_FACILITY_DIAG             \
IIR_FACILITY_DIAG                    \
IQUEUE_FACILITY_DIAG                 \
IR_RECEIVER_FACILITY_DIAG            \
IR_SAMSUNG_FACILITY_DIAG             \
ISO_TP_FACILITY_DIAG                 \
KALAH_FACILITY_DIAG                  \
LED_MONO_PWM_FACILITY_DIAG           \
LITTLE_FS_FACILITY_DIAG              \
LOCKSTEP_FACILITY_DIAG               \
LOG_FACILITY_DIAG                    \
LTR390_FACILITY_DIAG                 \
MAILBOX_FACILITY_DIAG                \
MAM_FACILITY_DIAG                    \
MAX_HEAP_FACILITY_DIAG               \
MEDIAN_FILTER_FACILITY_DIAG          \
MEDIAN_FILTER_FAST_FACILITY_DIAG     \
MICROCONTROLLER_FACILITY_DIAG        \
MIN_HEAP_FACILITY_DIAG               \
MPU_FACILITY_DIAG                    \
MULTICORE_FACILITY_DIAG              \
NAU8814_FACILITY_DIAG                \
PHASE_DETECTOR_FACILITY_DIAG         \
PID_FACILITY_DIAG                    \
PLL_CALC_FACILITY_DIAG               \
PLL_SIM_FACILITY_DIAG                \
POSTPONE_FUN_FACILITY_DIAG           \
PWM_DAC_FACILITY_DIAG                \
QUADRATURE_MIXER_FACILITY_DIAG       \
RC_CAR_FACILITY_DIAG                 \
RS232_FACILITY_DIAG                  \
RS485_FACILITY_DIAG                  \
RUNNING_LINE_FACILITY_DIAG           \
SA51034_FACILITY_DIAG                \
SCAN_FACILITY_DIAG                   \
SCRIPT_FACILITY_DIAG                 \
SED_FACILITY_DIAG                    \
SERIAL_PORT_FACILITY_DIAG            \
SET_GAME_FACILITY_DIAG               \
SLCAN_FACILITY_DIAG                  \
SMOOTH_LAMP_FACILITY_DIAG            \
SOCKET_FACILITY_DIAG                 \
SONAR_FACILITY_DIAG                  \
SOUND_LOCALIZATION_FACILITY_DIAG     \
SPIFI_FACILITY_DIAG                  \
STATISTICS_FACILITY_DIAG             \
STORAGE_FACILITY_DIAG                \
STORE_FS_FACILITY_DIAG               \
STRING_FACILITY_DIAG                 \
STRING_READER_FACILITY_DIAG          \
SW_COMPONENT_FACILITY_DIAG           \
SW_NOR_FLASH_FACILITY_DIAG           \
SW_UART_FACILITY_DIAG                \
TICKET_SET_OPT_FACILITY_DIAG         \
TIMER_CALC_FACILITY_DIAG             \
TOPO_SORT_FACILITY_DIAG              \
TRNG_FACILITY_DIAG                   \
UBLOX_NEO_6M_FACILITY_DIAG           \
UDS_CLIENT_FACILITY_DIAG             \
UDS_FACILITY_DIAG                    \
USB_DEVICE_FACILITY_DIAG             \
USB_SERIAL_FACILITY_DIAG             \
W25Q32JV_FACILITY_DIAG               \
WATCHDOG_FACILITY_DIAG               \
WAV_FACILITY_DIAG                    \
WIFI_FACILITY_DIAG
