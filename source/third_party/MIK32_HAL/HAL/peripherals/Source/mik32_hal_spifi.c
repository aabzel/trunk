...
void HAL_SPIFI_MemoryMode_Init(SPIFI_MemoryModeConfig_HandleTypeDef *spifi)
{
    ...
    spifi->Instance->STAT.dword |= SPIFI_CONFIG_STAT_RESET_M;
    spifi->Instance->CLIMIT = spifi->CacheLimit; // Граница кеширования
    if (spifi->CacheEnable)
    {
        spifi->Instance->CTRL.dword |= SPIFI_CONFIG_CTRL_CACHE_EN_M;
    }
    else
    {
        spifi->Instance->CTRL.dword &= ~SPIFI_CONFIG_CTRL_CACHE_EN_M;
...
HAL_StatusTypeDef HAL_SPIFI_SendCommand_LL(
    ...
    uint32_t timeout)
{
    spifi->Instance->STAT.dword |= SPIFI_CONFIG_STAT_INTRQ_M;
    spifi->Instance->ADDR = address;
    spifi->Instance->IDATA = interimData;
    spifi->Instance->CMD.dword = cmd | SPIFI_CONFIG_CMD_DATALEN(bufferSize);
    ...
    if (cmd & SPIFI_CONFIG_CMD_DOUT_M)
    {
        ...
        {
            spifi->Instance->DATA.data8[0] = writeBuffer[i];
        }
    }
    else
    {
        ...
        for (int i = 0; i < bufferSize; i++)
        {
            readBuffer[i] = (uint8_t)spifi->Instance->DATA.data8[0];
        }
    }
    ...
    {
        if (SPIFI_CONFIG->DATA.data8[0] == (cmd & SPIFI_CONFIG_CMD_POLL_REQUIRED_VALUE_M))
        {
            return HAL_OK;
        }
        return HAL_ERROR;
...
{
    return (spifi->Instance->STAT.dword & SPIFI_CONFIG_STAT_MCINIT_M) != 0;
}
#endif
...
{
    return (spifi->Instance->STAT.dword & SPIFI_CONFIG_STAT_RESET_M) == 0;
}
#endif
