...
typedef union {
    ...
    struct {
        uint32_t PE:1;  /**/
        uint32_t FE:1; /*Флаг взводится при обнаружении ошибок в стоп бите или битах. Флаг сбрасывается записью 1*/
        uint32_t NF:1; /*Флаг обнаружения ложных переключений на линии RX. Флаг сбрасывается записью 1*/
        uint32_t ORE:1; /*Флаг взводится при попытке перезаписи RDR. Флаг сбрасывается записью 1*/
        uint32_t IDLE:1; /*Флаг взводится при отсутствии активности на линии RX в течении 8 битовых тактов при взведенном флаге RXNE. Флаг сбрасывается записью 1*/
        uint32_t RXNE:1; /*Флаг взводится при записи принятых данных в регистр RDR из сдвигового регистра. Флаг сбрасывается записью 1 или чтением регистра RDR*/
        uint32_t TC:1; /*Флаг взводится при передаче данных в момент отправки последнего стоп бита. Флаг сбрасывается записью 1*/
        uint32_t TXE:1; /*Флаг взводится при передаче данных в момент записи данных в сдвиговый регистр. Флаг сбрасывается записью данных в регистр TDR или записью 1*/
        uint32_t LBDF:1; /*Флаг обнаружения break состояния на линии RX. Флаг сбрасывается записью 1*/
        uint32_t CTSIF:1; /*Флаг взводится при изменении значения сигнала CTS. Флаг сбрасывается записью 1*/
        uint32_t CTS:1; /*Флаг отображает текущее значение сигнала CTS*/
        uint32_t RES1:5; /**/
        uint32_t BUSY:1; /*Флаг активности на линии RX*/
        uint32_t RES2:4; /**/
        uint32_t TEACK:1; /*Флаг готовности передатчика к работе после снятия сброса*/
        uint32_t REACK:1; /*Флаг готовности приемника к работе после снятия сброса*/
        uint32_t RES3:9; /**/
    };
} __attribute__((__packed__)) Mik32UsartRegFLAGS_t;

// see Table 23 – Received Data Register RXDATA
typedef union {
    uint32_t dword;
    struct {
        uint32_t RDR:9;
...
} __attribute__((__packed__)) Mik32UsartRegRXDATA_t;
    ...
    {  
        ...
        volatile Mik32UsartRegFLAGS_t FLAGS;
        volatile uint32_t _reserved3;              
        volatile Mik32UsartRegRXDATA_t RXDATA;
