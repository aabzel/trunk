...
{
    if ((uart->FLAGS.dword & UART_FLAGS_TC_M) == 0)
    {
        return false;
    }
    else
...
{
    return ((uart->FLAGS.dword & UART_FLAGS_TXE_M) != 0);
}
...
{
    return ((uart->FLAGS.dword & UART_FLAGS_RXNE_M) == 0);
}
...
{
    return (uint16_t)uart->RXDATA.dword;
}
