...
typedef enum
{
    HAL_EEPROM_OP_READ = 0,
    HAL_EEPROM_OP_CLEAR = 1,
    HAL_EEPROM_OP_PROGRAM = 2,
} HAL_EEPROM_OperationTypeDef;

typedef enum
{
    HAL_EEPROM_WRITE_SINGLE = 0,
    HAL_EEPROM_WRITE_EVEN = 1,
    HAL_EEPROM_WRITE_ODD = 2,
    HAL_EEPROM_WRITE_ALL = 3,
} HAL_EEPROM_WriteBehaviourTypeDef;

typedef enum
{
    HAL_EEPROM_MODE_TWO_STAGE = 0,
    HAL_EEPROM_MODE_THREE_STAGE = 1,
} HAL_EEPROM_ModeTypeDef;

typedef enum
{
    HAL_EEPROM_ECC_ENABLE = 0,
    HAL_EEPROM_ECC_DISABLE = 1,
} HAL_EEPROM_ErrorCorrectionTypeDef;

typedef enum
{
    HAL_EEPROM_SERR_DISABLE = 0,
    HAL_EEPROM_SERR_ENABLE = 1,
...
typedef struct
{
	volatile EEPROM_REGS_TypeDef *Instance;
...
HAL_StatusTypeDef HAL_EEPROM_WaitBusy(HAL_EEPROM_HandleTypeDef *eeprom,
                                      uint32_t timeout)
{
    while (timeout)
    {
        timeout--;
        if (EEPROM_EESTA_BSY_FREE == eeprom->Instance->EESTA.BSY )
        {
            return HAL_OK;
        }
    }
