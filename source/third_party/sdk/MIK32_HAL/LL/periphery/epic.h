...
#define EPIC_LINE_M(interrupt_line)     (1 << interrupt_line)
    ...
    {  
        ...
        volatile uint32_t MASK_LEVEL_CLEAR;     // 0x0C   12
        volatile uint32_t reserved[2];          // reserved 16, 20
        volatile uint32_t CLEAR;                // 0x18    24
        volatile uint32_t STATUS;               // 0x1C   28
        volatile uint32_t RAW_STATUS;           // 0x20   32
    } EPIC_TypeDef;
#endif
