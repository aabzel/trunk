...
static uint32_t CanSynchroToSyncJumpWidth(const uint32_t synchronization) {
...
static uint32_t CanCanTxFrameToRtr(const CanTxFrame_t frame_type) {
...
static uint32_t CanPhase2ToTimeSeg2(const uint32_t phase_2) {
...
static uint32_t CanLoopBackToOperatingMode(const bool on_off) {
    uint32_t op_mode = CAN_MODE_NORMAL;
    if(on_off) {
        op_mode = CAN_MODE_LOOPBACK;
    } else {
...
static uint32_t CanPhase1ToTimeSeg1(const uint32_t phase_1) {
...
static bool can_proc_status_rx_fifo_one_ll(CanHandle_t* const Node, uint8_t fifo_num){
    ...
    if(RFxR.FULL) {
        LOG_ERROR(CAN, "CAN%u,FIFO%u,Full", Node->num, fifo_num);
    }

    if(RFxR.FOVR) {
        LOG_ERROR(CAN, "CAN%u,FIFO%u,OverRun", Node->num, fifo_num);
    }
...
static bool can_proc_status_ll(CanHandle_t* const Node) {
    ...
    if(CAN_MSR.TXM) {
        can_tx_indication(Node);
        LOG_DEBUG(CAN, "CAN%u,Tx", Node->num);
    }

    if(CAN_MSR.SLAK) {
        LOG_WARNING(CAN, "CAN%u,SleepAck", Node->num);
    }

    if(CAN_MSR.INAK) {
        LOG_WARNING(CAN, "CAN%u,Initialization,ack", Node->num);
    }


    if(CAN_MSR.WKUI) {
        LOG_WARNING(CAN, "CAN%u,Wake-up,Int", Node->num);
    }
...
static bool can_config_to_init(const CanConfig_t* const Config, CAN_InitTypeDef* const Init) {
    bool res = false;
    if(Config) {
        if(Init) {
            uint32_t bus_freq_hz = 0;
...
static bool can_init_filters(const CanConfig_t* const Config, CanHandle_t* const Node) {
    bool res = true;
...
static bool can_init_interrupt(CanHandle_t* const Node) {
    bool res = false;
    HAL_StatusTypeDef ret = HAL_ERROR;
