...
static bool i2c_master_check_addr2(I2C_HandleTypeDef* hi2c, uint16_t DevAddress){
    bool res = false;
    HAL_StatusTypeDef status = HAL_I2C_IsDeviceReady(hi2c, DevAddress, 2, 1000);
    res = HAL_retToRes(status);
    status = HAL_I2C_Master_Abort_IT(hi2c, DevAddress);
    return res;
}
...
bool i2c_check2_addr(uint8_t num, uint8_t i2c_addr) {
    ...
    I2cHandle_t* Node = I2cGetNode(num);
    if(Node) {
        res = i2c_master_check_addr2(&Node->i2c_h, i2c_addr);
    }
    return res;
}
