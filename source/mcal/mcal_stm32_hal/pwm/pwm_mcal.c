...
static int32_t TimerPhaseUsToCompareValue(uint8_t num, int32_t phase_us) {
...
static PwmPolarity_t PwmPolarityToggle(const PwmPolarity_t prev_polarity) {
    ...
    case PWM_POLARITY_LOW:
        out_polarity = PWM_POLARITY_HIGH;
        break;
    case PWM_POLARITY_HIGH:
        out_polarity = PWM_POLARITY_LOW;
        break;
    default:
        out_polarity = PWM_POLARITY_LOW;
        break;
...
bool pwm_phase_set_sw(uint8_t num, int32_t phase_us) {
    ...
    if(Node) {
        ...
        LOG_INFO(PWM, "SetPhaSW:%d us,compareValue:%d,CNTMaster:%u,CNTslave:%u", phase_us, compare_value, counter_base,
                 value);
    }
    return res;
}
...
bool pwm_phase_set_hw(const uint8_t num, const int32_t phase_us) {
    ...
    if(Node) {
        int32_t compare_value = TimerPhaseUsToCompareValue(Node->PhaseComparator.timer, phase_us);
        res = timer_compare_set(Node->PhaseComparator.timer, Node->PhaseComparator.channel, compare_value);
        LOG_DEBUG(PWM, "SetPhaHW:%u us,compareValue:%u,PHAcmp:%s", phase_us, compare_value,
                  TimChanToStr(Node->PhaseComparator));
    }
    return res;
}
