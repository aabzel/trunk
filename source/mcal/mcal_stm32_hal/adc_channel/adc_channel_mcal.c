...
bool adc_channel_init(uint8_t adc_num, AdcChannel_t channel) {
    ...
    if (AdcCh) {
        ...
        LOG_WARNING(ADC_CHANNEL, "%s", AdcChannelConfigToStr(AdcCh));
#endif
...
bool adc_channel_init_one(uint8_t num) {
    ...
    if (Config) {
        ...
        if (Node) {
            ...
            if(res) {
                res = adc_channel_init(Config->AdcCh.adc, Config->AdcCh.channel);
                res = gpio_init_adc_pad(Config->Pad);
                Node->valid = true;
            }
        }
    }
...
static bool adc_channel_interrupt_proc_one_ll(AdcChannelHandle_t* AdcCh) {
    bool res = false;
    res = adc_channel_read_code(AdcCh->AdcCh.adc, (AdcChannel_t) AdcCh->AdcCh.channel, (int32_t*) &AdcCh->code);
    if(res) {
        adc_code_to_params(AdcCh);
    } else {
        LOG_DEBUG(ADC_CHANNEL, "ReadErr,Adc:%u,Ch:%u", AdcCh->num, AdcCh->AdcCh.channel);
    }
    return res;
}

bool adc_channel_interrupt_proc_one(uint8_t num) {
    bool res = false;
    AdcChannelHandle_t *AdcCh = AdcChannelGetNode(num);
    if(AdcCh) {
        res = adc_channel_interrupt_proc_one_ll(AdcCh);
    }
    return res;
}
