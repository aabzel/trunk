...
bool adc_custom_in_diag(char* key_word1, char* key_word2) {
    ...
    for(i = 0; i < chan_cnt; i++) {
        ...
        snprintf(log_line, sizeof(log_line), "%s %4s " TSEP, log_line, AdcChannelToStr(AdcChannelInstance[i].AdcCh.channel));
        ...
        const AdcChannelConfig_t* AdcChConfig =
            AdcChannelGetConfigV2(AdcChannelInstance[i].num, AdcChannelInstance[i].AdcCh.channel);
        if(AdcChConfig) {
