...
bool rtc_internal_init_one(uint8_t num) {
    ...
    if(Node) {
        ...
        if(HAL_OK == ret) {
            ...
            LOG_ERROR(LG_RTC, "Init,Internal,Err:%u=[%s]", ret, HalStatusToStr(ret));
            res = false;
        }
    }
    return res;
...
bool rtc_internal_set_time(uint8_t num, const struct tm* const DateTime) {
    ...
    if(Node) {
        ...
        if(HAL_OK == ret) {
            ...
            LOG_ERROR(LG_RTC, "Set,Time,Err:%s", HalStatusToStr(ret));
            res = false;
        }
    }
...
static uint8_t RtcMotchToStmHalMonth(int tm_mon) {
    ...
    case 11:
        ...
        break;
    default:
        LOG_ERROR(LG_RTC, "Month,Err,%u", tm_mon);
        break;
    }
...
static uint8_t RtcWeekDayToStmHalWeekDay(int tm_wday) {
    ...
    switch(tm_wday) {
    case 0:
        stm_hal_month = RTC_WEEKDAY_SUNDAY;
        break;
    case 1:
        stm_hal_month = RTC_WEEKDAY_MONDAY;
        break;
    case 2:
        stm_hal_month = RTC_WEEKDAY_TUESDAY;
        break;
    case 3:
        stm_hal_month = RTC_WEEKDAY_WEDNESDAY;
        break;
    case 4:
        stm_hal_month = RTC_WEEKDAY_THURSDAY;
        break;
    case 5:
        stm_hal_month = RTC_WEEKDAY_FRIDAY;
        break;
    case 6:
        stm_hal_month = RTC_WEEKDAY_SATURDAY;
        break;
    default:
        LOG_ERROR(LG_RTC, "WeekDay,Err");
        break;
    }
...
bool rtc_internal_set_date(uint8_t num, const struct tm* const DateTime) {
    ...
    if(Node) {
        ...
        if(HAL_OK == ret) {
            ...
            LOG_ERROR(LG_RTC, "Set,Date,Err:%s", HalStatusToStr(ret));
            res = false;
        }
    }
    return res;
        ...
        if(DateTime) {
            ...
            if(HAL_OK == ret) {
                ...
                if(HAL_OK == ret) {
                    ...
                    LOG_ERROR(LG_RTC, "Get,Date,Err:%s", HalStatusToStr(ret));
                    res = false;
                }
            } else {
                LOG_ERROR(LG_RTC, "Get,Time,Err:%s", HalStatusToStr(ret));
                res = false;
            }
        } else {
