...
bool interrupt_custom_diag(char* key_word1, char* key_word2) {
    ...
    for(irq_n = NonMaskableInt_IRQn; irq_n <= MAX_IRQ_NUM; irq_n++) {
        ...
        snprintf(temp_str, sizeof(temp_str), "%s %1u " TSEP, temp_str, (status));
        ...
        snprintf(temp_str, sizeof(temp_str), "%s %1ua   " TSEP, temp_str,
                 (unsigned int)HAL_NVIC_GetActive((IRQn_Type)irq_n));
        snprintf(temp_str, sizeof(temp_str), "%s %1ud   " TSEP, temp_str,
                 (unsigned int)HAL_NVIC_GetPendingIRQ((IRQn_Type)irq_n));
        ...
        snprintf(temp_str, sizeof(temp_str), "%s %7s " TSEP, temp_str, IntPriToStr(PreemptPriority));
        snprintf(temp_str, sizeof(temp_str), "%s %7s " TSEP, temp_str, IntPriToStr(SubPriority));
#ifdef HAS_INTERRUPT_DIAG
        snprintf(temp_str, sizeof(temp_str), "%s %15s " TSEP, temp_str, InterruptToStr(irq_n));
#endif
