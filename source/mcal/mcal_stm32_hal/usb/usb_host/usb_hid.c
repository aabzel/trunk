...
#ifdef HAS_KEYLOG
                uint8_t buff[80] = {0};
                res = tbfp_compose_frame((uint8_t*)&KeyBoard, (uint16_t)sizeof(KeyBoard_t), IF_FAT_FS,
                                         FRAME_ID_KEYBOARD, buff, sizeof(buff));
                if(res) {
                    LOG_DEBUG(KEYLOG, "ComposeOk");
                    res = fifo_push_array(&KeyLog.Fifo, (char*)buff, sizeof(KeyBoard_t) + TBFP_OVERHEAD_SIZE);
                    if(res) {
