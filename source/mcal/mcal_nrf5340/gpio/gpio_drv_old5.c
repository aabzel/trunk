...
bool gpio_set_pin_mcu(Pad_t Pad, NrfGpioPinMcuSel_t mcu_sel) {
    bool res = false;
...
bool gpio_init_one(const GpioConfig_t* pinInstance) {
    bool res = false;
#ifdef HAS_LOG
    LOG_DEBUG(GPIO, "InitOne Dir:%s", GpioDir2Str(pinInstance->dir));
#endif
    res = gpio_is_pin_single(pinInstance->pad.byte);
    if(res) {
        if(pinInstance) {
            switch(pinInstance->dir) {
            case GPIO_DIR_INOUT:
            case GPIO_DIR_OUT_PUSH_PULL:
            case GPIO_DIR_OUT: {
                nrf_gpio_pin_pull_t pull_config = GpioPull2NrfPull(pinInstance->gpio_pull);
                nrf_gpio_cfg_input((uint32_t)pinInstance->pad.byte, pull_config);
                nrf_gpio_cfg_output(pinInstance->pad.byte);
                gpio_set_state(pinInstance->pad.byte, (uint8_t)pinInstance->logic_level);
                res = true;
            } break;
            case GPIO_DIR_IN: {
            } break;
            default:
                LOG_WARNING(GPIO, "UndefDir %s Dir:%s", GpioPad2Str(pinInstance->pad.byte),
                            GpioDir2Str(pinInstance->dir));
                res = true;
                break;
            }
            if(NRF_GPIO_PIN_MUX_UNDEF != pinInstance->mcu_sel) {
                gpio_set_pin_mcu(pinInstance->pad, pinInstance->mcu_sel);
            }
            //
            res = gpio_set_pull(pinInstance->pad.byte, pinInstance->gpio_pull);
        }
    } else {
#ifdef HAS_GPIO_DIAG
        LOG_ERROR(GPIO, "ReDefine Pad:%s", GpioPad2Str(pinInstance->pad.byte));
#endif /*HAS_GPIO_DIAG*/
    }
    return res;
}

// gpio_toggle(NRF_GPIO_PIN_MAP(0,30));
