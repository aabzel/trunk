...
bool flash_mcal_write(uint32_t address, uint8_t* wr_array, uint32_t num_bytes) {
    LOG_DEBUG(LG_FLASH, "Wrire Addr:0x%08x Size %u byte", address, num_bytes);
    bool res = false;
...
#ifdef HAS_FLASH_WRITE
bool flash_mcal_writeite_dword(uint32_t addr, uint32_t* wr_array, size_t size) {
    bool res = true;
...
uint32_t flash_nrfx_get_first_spare_page(void) {
    bool res = false;
...
#ifdef HAS_CRC16
    FlashInstance.crc16 = calc_crc16_ccitt_false((const uint8_t* const)APP_START_ADDRESS, ON_CHIP_NOR_FLASH_SIZE);
    LOG_INFO(LG_FLASH, "AllFlash CRC16 0x%x", FlashInstance.crc16);
#endif
    FlashInstance.start = APP_START_ADDRESS;
    FlashInstance.size = nrfx_nvmc_flash_size_get();
    FlashInstance.page_size = nrfx_nvmc_flash_page_size_get();
    FlashInstance.page_cnt = nrfx_nvmc_flash_page_count_get();
    FlashInstance.end = APP_START_ADDRESS + FlashInstance.size;
#ifdef HAS_FLASH_DIAG
