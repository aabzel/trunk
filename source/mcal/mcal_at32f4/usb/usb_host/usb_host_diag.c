...
const char* UsbHostSpeed2Str(USBH_SpeedTypeDef usbh_speed){
	...
	case USBH_SPEED_HIGH: name="Hi"; break;
	case USBH_SPEED_LOW: name="Low"; break;
	case USBH_SPEED_FULL: name="Full"; break;
	}
	return name;
}
...
const char* UsbHidType2Str(HID_TypeTypeDef hid_type){
	...
	switch((uint8_t) hid_type){
    	case HID_MOUSE: name="Mouse"; break;
	    case HID_KEYBOARD: name="KeyBoard"; break;
...
const char* KeyBoardInfo2Str(const HID_KEYBD_Info_TypeDef* const Node){
	...
	if(Node){
        snprintf(name,sizeof(name),"state:%u,lctrl:%u,lshift:%u,lalt:%u,lgui:%u,rctrl:%u,rshift:%u,ralt:%u,rgui:%u",
        		Node->state,Node->lctrl, Node->lshift, Node->lalt,
        		Node->lgui, Node->rctrl, Node->rshift, Node->ralt,
        		Node->rgui);
	}
	return name;
}
...
const char* UsbHostStatus2Str( USBH_StatusTypeDef status){
	...
	switch((uint8_t) status){
    	case USBH_OK: name="Ok"; break;
	    case USBH_BUSY: name="Busy"; break;
	    case USBH_FAIL: name="FAIL"; break;
	    case USBH_NOT_SUPPORTED: name="NOT_SUPPORTED"; break;
	    case USBH_UNRECOVERED_ERROR: name="UNRECOVERED_ERROR"; break;
	    case USBH_ERROR_SPEED_UNKNOWN: name="ERROR_SPEED_UNKNOWN"; break;
...
bool usb_host_diag(void) {
    ...
    if(Node){
        ...
        if(phost){
        	...
        	USBH_StatusTypeDef status = USBH_HID_GetReport(phost,
        	                                        0x02U,
        	                                        0,
        	                                      (uint8_t *)kbd_led,
        	                                        sizeof(kbd_led));
        	LOG_INFO(USB_HOST, "%s", UsbHostStatus2Str(status));
            res = array_i8_print(kbd_led, 2);
            ...
            if(KeyBoardNode ) {
                ...
                res = array_i8_print(KeyBoardNode->keys, 8);
            }

            status = USBH_Get_StringDesc(phost, phost->device.DevDesc.iManufacturer,
