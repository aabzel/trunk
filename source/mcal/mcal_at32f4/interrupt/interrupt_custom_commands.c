...
bool interrupt_custom_set_command(int32_t argc, char* argv[]) {
    ...
    if(3 == argc) {
        ...
        if(false == res) {
            LOG_ERROR(LG_INT, "Unable to parse PreemptPriority");
        }
    }
    ...
    if(res) {
        ...
        LOG_ERROR(LG_INT, "Usage: is irq_n PreemptPriority SubPriority");
    }
    return res;
}
...
bool interrupt_custom_enable_irq_command(int32_t argc, char* argv[]) {
    ...
    if(res && (2 == argc)) {
        if(on_off) {
            LOG_INFO(LG_INT, "Ok %d %u Enable", irq_n, on_off);
            NVIC_EnableIRQ((IRQn_Type)irq_n);
        } else {
            LOG_INFO(LG_INT, "Ok %d %u Disable", irq_n, on_off);
...
static bool interrupt_custom_diag(char* key_word1, char* key_word2) {
    bool res = false;

    uint32_t handler_addr = 0;
    static const table_col_t cols[] = {
        {5, "No"},     {5, "irq"},     {3, "En"},     {12, "Base"},
        {6, "Active"}, {6, "pending"}, {6, "PrePri"}, {6, "SubPri"},
#ifdef HAS_NVIC_DIAG
        {9, "PrePri"}, {9, "SubPri"},
#endif
...
bool interrupt_custom_diag_command(int32_t argc, char* argv[]) {
    ...
    if(true == res) {
        res = interrupt_custom_diag(keyWord1, keyWord2);
    }
    return res;
}
...
bool interrupt_custom_enable_command(int32_t argc, char* argv[]) {
    ...
    if(1 == argc) {
        ...
        if(false == res) {
            LOG_ERROR(LG_INT, "Unable to parse status");
        }
    }
    if(res) {
        if(false == status) {
