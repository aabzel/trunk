...
static GpioPullMode_t GpioAt32Pull2GeneralPull(GpioAt32Pull_t stm32_pull_code) {
...
GpioAt32Mode_t gpio_get_mode(uint8_t pad_num) {
    GpioAt32Mode_t pin_mode = GP_STM32_MODE_UNDEF;
    Pad_t pad;
    pad.byte = pad_num;
    GPIO_TypeDef* GPIOx = Port2PortPtr(pad.port);
    if(GPIOx) {
...
GpioAt32Speed_t gpio_get_speed(uint8_t pad_num) {
    ...
    if(GPIOx) {
        ...
        code = MASK_2BIT & (reg_mode >> (2 * pad.pin));
        mode = (GpioAt32Speed_t)code;
    }
    return mode;
}

static GpioAt32Pull_t GpioPull2At32Pull(GpioPullMode_t pull_code) {
    GpioAt32Pull_t stm32_pull_code = GPIO__PULL_UNDEF;
...
GpioPullMode_t gpio_get_pull(uint8_t pad_num) {
    GpioAt32Pull_t mode = STM32_PULL_UNDEF;
    Pad_t pad;
    pad.byte = pad_num;
    GPIO_TypeDef* GPIOx = Port2PortPtr(pad.port);
    if(GPIOx) {
        ...
        code = MASK_2BIT & (reg_mode >> (2 * pad.pin));
        mode = GpioAt32Pull2GeneralPull((GpioAt32Pull_t)code);
    } else {
#ifdef HAS_GPIO_DIAG
...
bool gpio_set_pull(uint8_t pad_num, GpioPullMode_t pull_mode) {
    bool res = false;
    Pad_t pad;
    GpioAt32Pull_t stm_pull_mode = GpioPull2At32Pull(pull_mode);
    pad.byte = pad_num;
    GPIO_TypeDef* GPIOx = Port2PortPtr(pad.port);
    if(GPIOx) {
        uint32_t reg_mode = GPIOx->PUPDR;
    ...
    if(Inst) {
        ...
        if(res) {
            ...
            if(GpioPort) {
                ...
                GPIO_InitStruct.Pull = GpioPull2At32Pull(Inst->gpio_pull);
