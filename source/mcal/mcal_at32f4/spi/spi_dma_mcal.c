...
static uint8_t spi_tx_to_dma_mux(uint8_t num) {
    uint8_t mux = 0;
    switch(num) {
    case 1:
...
static uint8_t spi_rx_to_dma_mux(uint8_t num) {
    uint8_t mux = 0;
    switch(num) {
    case 1:
...
bool spi_dma_ctrl_ll(SpiHandle_t* Node, bool on_off) {
    ...
    case I2S_DIR_BUS_MODE_MASTER_RX: {
        // I in RX
        spi_spi_dma_receiver_enable(Node->SPIx, new_state);
        res = true;
    } break;
    ...
    case I2S_DIR_BUS_MODE_MASTER_TX: {
        // I Out TX
        spi_spi_dma_transmitter_enable(Node->SPIx, new_state);
        res = true;
    } break;
...
bool spi_dma_ctrl(uint8_t num, bool on_off) {
    ...
    if(Node) {
        res = spi_dma_ctrl_ll(Node, on_off);
    }
    return res;
}
...
bool i2s_init_dma(const I2sConfig_t* Config, SpiHandle_t* Node) {
    ...
    if(ChannelInfo) {
        ...
        case I2S_DIR_BUS_MODE_MASTER_TX:
        case I2S_DIR_BUS_MODE_SLAVE_TX: {
            dmamux_req_sel = spi_tx_to_dma_mux(Config->num);
        } break;
        case I2S_DIR_BUS_MODE_MASTER_RX:
        case I2S_DIR_BUS_MODE_SLAVE_RX: {
            dmamux_req_sel = spi_rx_to_dma_mux(Config->num);
        } break;
        }
        dmamux_init(ChannelInfo->dmamux_channelx, dmamux_req_sel);
        dma_channel_enable(ChannelInfo->dmax_channely, TRUE);
        res = spi_dma_ctrl_ll(Node, false);
    }
    return res;
}
...
bool i2s_api_write_dma(uint8_t num, SampleType_t* const array, size_t size, DmaMode_t mode) {
    ...
    if(Node) {
        ...
        spi_spi_dma_transmitter_enable(Node->SPIx, FALSE);
        ...
        Config.mux = spi_tx_to_dma_mux(num);
        ...
        Config.peripheral_base_addr = (uint32_t) & (Node->SPIx->dt);
        ...
        spi_spi_dma_transmitter_enable(Node->SPIx, TRUE);
...
bool i2s_api_read_dma(uint8_t num, SampleType_t* const array, size_t size, DmaMode_t mode) {
    ...
    if(Node) {
        ...
        spi_spi_dma_receiver_enable(Node->SPIx, FALSE);
        DmaChannelConfig_t Config;
        Config.num = 4;
        ...
        Config.mux = spi_rx_to_dma_mux(num);       // affective
        Config.periph_burst = DMA_BURST_SINGLE;
        Config.per_inc = DMA_INC_OFF;          // affective
        Config.priority = DMA_PRIOR_VERY_HIGH; // affective
        Config.valid = true;
        Config.peripheral_base_addr = (uint32_t) & (Node->SPIx->dt); // affective
        Config.stream_num = Node->Dma.channel;
        spi_i2s_interrupt_enable(Node->SPIx, SPI_I2S_RDBF_INT, FALSE);
        spi_spi_dma_receiver_enable(Node->SPIx, TRUE);
        spi_enable(Node->SPIx, TRUE);
        res = dma_channel_init_one_ll(&Config);
        if(res) {
