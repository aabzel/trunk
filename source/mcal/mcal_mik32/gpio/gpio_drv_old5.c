...
static GpioPullMode_t GpioMik32Pull2GeneralPull(GpioMik32Pull_t stm32_pull_code) {
    ...
    case MIK32_PULL_UP:
        ...
        break;
    case MIK32_PULL_DOWN:
        general_pull_code = GPIO__PULL_DOWN;
        break;
    case MIK32_PULL_UNDEF:
...
GpioDir_t gpio_get_dir(uint8_t pad_num) {
    ...
    GpioMik32Mode_t mode = gpio_get_mode(pad_num);
    switch((uint8_t)mode) {
...
GpioMik32Mode_t gpio_get_mode(uint8_t pad_num) {
    GpioMik32Mode_t pin_mode = GP_MIK32_MODE_UNDEF;
    Pad_t pad;
    pad.byte = pad_num;
    GPIO_TypeDef* GPIOx = Port2PortPtr(pad.port);
    if(GPIOx) {
...
GpioMik32Speed_t gpio_get_speed(uint8_t pad_num) {
    ...
    if(GPIOx) {
        ...
        code = MASK_2BIT & (reg_mode >> (2 * pad.pin));
        mode = (GpioMik32Speed_t)code;
    }
    return mode;
}

static GpioMik32Pull_t GpioPull2Mik32Pull(GpioPullMode_t pull_code) {
    GpioMik32Pull_t stm32_pull_code = GPIO__PULL_UNDEF;
    switch((uint8_t)pull_code) {
    case GPIO__PULL_AIR:
        stm32_pull_code = MIK32_PULL_AIR;
        break;
    case GPIO__PULL_UP:
        stm32_pull_code = MIK32_PULL_UP;
        break;
    case GPIO__PULL_DOWN:
        stm32_pull_code = MIK32_PULL_DOWN;
        break;
    case GPIO__PULL_UNDEF:
        stm32_pull_code = MIK32_PULL_UNDEF;
        break;
    }
...
GpioPullMode_t gpio_get_pull(uint8_t pad_num) {
    GpioMik32Pull_t mode = MIK32_PULL_UNDEF;
    Pad_t pad;
    pad.byte = pad_num;
    GPIO_TypeDef* GPIOx = Port2PortPtr(pad.port);
    if(GPIOx) {
        ...
        code = MASK_2BIT & (reg_mode >> (2 * pad.pin));
        mode = GpioMik32Pull2GeneralPull((GpioMik32Pull_t)code);
    } else {
#ifdef HAS_GPIO_DIAG
...
bool gpio_set_pull(uint8_t pad_num, GpioPullMode_t pull_mode) {
    bool res = false;
    Pad_t pad;
    GpioMik32Pull_t stm_pull_mode = GpioPull2Mik32Pull(pull_mode);
    pad.byte = pad_num;
    GPIO_TypeDef* GPIOx = Port2PortPtr(pad.port);
    if(GPIOx) {
        uint32_t reg_mode = GPIOx->PUPDR;
    ...
    if(Inst) {
        ...
        if(res) {
            ...
            if(GpioPort) {
                ...
                GPIO_InitStruct.Pull = GpioPull2Mik32Pull(Inst->gpio_pull);
