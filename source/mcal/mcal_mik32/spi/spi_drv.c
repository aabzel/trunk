...
static uint32_t SpiPhase2Mik32Phase(SpiPhase_t phase) {
...
uint32_t SpiPolarity2Mik32Polarity(SpiPolarity_t polarity) {
    ...
    if(Config) {
        ...
        if(Node) {
            ...
            if(SpiInstance) {
                ...
                Node->handle.Init.CLKPolarity = SpiPolarity2Mik32Polarity(Config->polarity);
                Node->handle.Init.CLKPhase = SpiPhase2Mik32Phase(Config->phase);
