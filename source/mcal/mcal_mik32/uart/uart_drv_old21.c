...
bool uart_init_ll(uint8_t uart_num) {
    ...
    if(UartNode) {
        ...
        if(UartConfNode) {
            if(false == UartNode->RxFifo.initDone) {
            ...
            // res=fifo_init(&UartNode->TxFifo, UartNode->tx_array, UART_TX_FIFO_SIZE);
            UartNode->tx_done = true;
            UartNode->rx_done = true;
            UartNode->rx_buff = (volatile uint8_t*)&rx_buff[uart_num][0];
            UartNode->rx_buff_size = 1;
            UartNode->uart_h.Init.BaudRate = UartConfNode->baud_rate;
            UartNode->uart_h.Init.HwFlowCtl = UART_HWCONTROL_NONE;
            UartNode->uart_h.Init.Mode = UART_MODE_TX_RX;
            UartNode->uart_h.Init.OverSampling = UART_OVERSAMPLING_16;
            UartNode->uart_h.Init.Parity = UART_PARITY_NONE;
            UartNode->uart_h.Init.StopBits = UART_STOPBITS_1;
            UartNode->uart_h.Init.WordLength = UART_WORDLENGTH_8B;
            UartNode->uart_h.Instance = UartGetBase(uart_num);
            HAL_StatusTypeDef ret = HAL_ERROR;
            ret = HAL_UART_Init(&UartNode->uart_h);
            if(HAL_OK != ret) {
                res = false;
            ...
            ret = HAL_UART_Receive_IT(&UartNode->uart_h, (uint8_t*)UartNode->rx_buff, UartNode->rx_buff_size);
            if(HAL_OK != ret) {
                res = false;
            }
            char str[40] = "UART";
...
/*see Figure 4. MIK3213xG/H block diagram*/
...
static bool uart_wait_tx_done_ll(UartHandle_t* UartNode) {
    ...
    if(UartNode) {
        ...
        uint8_t uart_num = get_uart_index(UartNode->uart_h.Instance);
        ...
        uint32_t cur_us = 0;
        if(UartNode->tx_len) {
            time_out_us = calc_transfer_time_us(baudrate, (uint32_t)UartNode->tx_len + 1);
        ...
        do {
            ...
            if(HAL_UART_STATE_READY == UartNode->uart_h.gState) {
                res = false;
                break;
            }
...
bool uart_wait_send_ll(uint8_t uart_num, uint8_t* tx_buffer, uint16_t len) {
    ...
    UartHandle_t* UartNode = UartGetNode(uart_num);
    if(UartNode) {
        if(UartNode->init_done && (len) && tx_buffer) {
            res = uart_wait_tx_done_ll(UartNode);
            if(res) {
                UartNode->tx_done = false;
                UartNode->tx_len = len;
                ret = HAL_UART_Transmit_IT(&UartNode->uart_h, tx_buffer, len);
                if(HAL_OK == ret) {
                    res = true;
                } else {
...
bool uart_send_wait_ll(uint8_t uart_num, uint8_t* tx_buffer, uint16_t len) {
    ...
    if(UartNode) {
        if(UartNode->init_done && (len) && tx_buffer) {
            uint32_t init_tx_cnt = UartNode->tx_cnt;
#ifdef HAS_UART_TX_TIMEOUT
            uint32_t baudrate = uart_get_cfg_baudrate(uart_num);
            ...
            uint32_t cur_us = 0;
            uint32_t start_us = time_get_us();
            stat = HAL_UART_Transmit_IT(&UartNode->uart_h, tx_buffer, len);
            if(HAL_OK == stat) {
                res = true;
                // We send mainly from Stack. We need wait the end of transfer.
                // Otherwise tx data will not be valid
                while(init_tx_cnt == UartNode->tx_cnt) {
                    cur_us = time_get_us();
                    dutation_us = cur_us - start_us;
#ifdef HAS_UART_TX_TIMEOUT
                ...
                UartNode->real_byte_tx_time_us = dutation_us / len;
            }
...
void HAL_UART_RxCpltCallback(UART_HandleTypeDef* uart_handle) {
    ...
    if(UartNode) {
        UartNode->rx_cnt++;
        UartNode->rx_done = true;
        HAL_UART_Receive_IT(&UartNode->uart_h, (uint8_t*)UartNode->rx_buff, 1);
#ifdef HAS_CLI
        if(UART_NUM_CLI == uart_num) {
            uart_string_reader_rx_callback(&cmd_reader, UartNode->rx_buff[0]);
        }
#endif /*HAS_CLI*/
#ifdef HAS_BC127
        if(UART_NUM_BC127 == uart_num) {
            bool res = fifo_push(&UartNode->RxFifo, UartNode->rx_buff[0]);
            if(false == res) {
...
#ifdef HAS_HEAP
        bool res = h_free((void*)UartNode->tx_buff);
        if(false == res) {
            UartNode->err_heap++;
        }
#endif
