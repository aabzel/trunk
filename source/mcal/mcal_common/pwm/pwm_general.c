...
int32_t pwm_phase_deg_to_phase_us(uint8_t num, float phase_deg) {
    int32_t phase_us = 0;
    bool res = false;
    float period_s = 0.0f;
...
PwmHandle_t* PwmTimerNumToNode(uint8_t timer_num) {
    ...
    for(i = 0; i < cnt; i++) {
        if(timer_num == PwmInstance[i].TimChan.timer) {
...
bool PwmIsValidConfig(const PwmConfig_t* const Config) {
    ...
    if(Config) {
        ...
        res = timer_is_valid(Config->TimChan.timer);
        ifn(res) {
            LOG_ERROR(PWM, "PWM%u,TimerNum,Err", Config->num);
            out_res = false;
        }

        res = timer_is_valid_channel(Config->TimChan.timer, Config->TimChan.channel);
        ifn(res) {
...
_WEAK_FUN_ bool pwm_phase_set(uint8_t num, int32_t phase_us) {
    bool res = false;
    LOG_ERROR(PWM, "[%s]NotImplemented", __FUNCTION__);
    return res;
}

_WEAK_FUN_ bool pwm_polarity_set(uint8_t num, PwmPolarity_t polarity) {
    bool res = false;
    LOG_ERROR(PWM, "[%s]NotImplemented", __FUNCTION__);
    return res;
}
...
_WEAK_FUN_
bool pwm_period_get(uint8_t num, float* const period_s) {
    float frequency_hz = 0.0f;
    bool res = pwm_frequency_get(num, &frequency_hz);
    if(res) {
        res = false;
        if(0 <= frequency_hz) {
            *period_s = 1.0f / frequency_hz;
            res = true;
        }
    }
    return res;
