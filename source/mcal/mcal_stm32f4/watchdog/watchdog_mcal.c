...
static bool iwdg_find_timing(uint32_t period_ms, uint32_t* out_prescaler, uint32_t* out_reload) {
    bool res = false;
    if(out_prescaler && out_reload) {
        uint32_t prescaler = 1;
        uint32_t reload = 0;
        float tick_time = 0.0;
        float cur_period_s = 0.0;
        float des_period_s = ((float)period_ms) / 1000.0;
        for(prescaler = 4; prescaler <= 256; prescaler *= 2) {
            for(reload = 1; reload <= RELOAD_VAL_MAX - 2; reload++) {
                tick_time = ((float)prescaler) / ((float)LSI_FREQ);
                cur_period_s = tick_time * ((float)reload);
                res = is_float_equal_absolute(cur_period_s, des_period_s, 0.00001);
                if(res) {
...
bool iwdg_set_period_ms(uint32_t period_ms) {
    ...
    if(res) {
        ...
        if(HAL_OK == ret) {
            ...
            LOG_ERROR(LG_IWDG, "SetErr %s", HalStatusToStr(ret));
...
bool watchdog_ctrl(bool on_off) {
    bool res = false;
    if(on_off) {
        res = iwdg_set_period_ms((uint32_t) (WatchDogConfig.timeout_s * 1000.0));
    }
    return res;
}

float iwdg_calc_period_s(uint16_t clock_hz, uint16_t prescaler, uint16_t reload) {
    float period_s = 0.0;
...
bool watchdog_mcal_init(void) {
    ...
    LOAD_PARAM(LG_IWDG, PAR_ID_WATCHDOG_BOOTLOADER_ON, WatchDog.is_on, false);
#endif

#if defined(HAS_GENERIC) && defined(HAS_PARAM)
    LOAD_PARAM(LG_IWDG, PAR_ID_WATCHDOG_GENERIC_ON, WatchDog.is_on, false);
#endif

#ifdef HAS_PARAM
    LOAD_PARAM(LG_IWDG, PAR_ID_WATCHDOG_TIME_OUT_MS, WatchDog.period_ms, IWDG_TIMEOUT_MS);
#endif
