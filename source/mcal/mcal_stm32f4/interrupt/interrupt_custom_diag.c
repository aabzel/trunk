...
bool interrupt_custom_diag(char* key_word1, char* key_word2) {
    ...
    for(irq_n = NonMaskableInt_IRQn; irq_n <= MAX_IRQ_NUM; irq_n++) {
            ...
            snprintf(temp_str, sizeof(temp_str), "%s %1ua   " TSEP, temp_str, (unsigned int)HAL_NVIC_GetActive((IRQn_Type) irq_n));
            snprintf(temp_str, sizeof(temp_str), "%s %1ud   " TSEP, temp_str, (unsigned int)HAL_NVIC_GetPendingIRQ((IRQn_Type) irq_n));
