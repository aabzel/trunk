...
ifneq ($(USB_HOST_DRV_MK_INC),Y)
    ...
    USB_CUSTOM_HOST_DIR = $(USB_CUSTOM_DIR)/usb_host
    #@echo $(error USB_CUSTOM_HOST_DIR=$(USB_CUSTOM_HOST_DIR))

    INCDIR += -I$(USB_CUSTOM_HOST_DIR)
    INCDIR += -I$(USB_CUSTOM_HOST_DIR)/Target

    #ifeq ($(USB_HOST_FS),Y)
    #    SOURCES_C += $(USB_CUSTOM_HOST_DIR)/Target/usbh_fs_conf.c
    #endif

    #ifeq ($(USB_HOST_HS),Y)
    #    SOURCES_C += $(USB_CUSTOM_HOST_DIR)/Target/usbh_hs_conf.c
    #endif
    SOURCES_C += $(USB_CUSTOM_HOST_DIR)/Target/usbh_conf.c
    SOURCES_C += $(USB_CUSTOM_HOST_DIR)/Target/usbh_platform.c

    SOURCES_C += $(USB_CUSTOM_HOST_DIR)/usb_host.c
