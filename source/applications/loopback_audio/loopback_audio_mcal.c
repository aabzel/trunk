...
bool loopback_audio_init_custom(void) {
    ...
    float duration_s = audio_samples_to_duration_s(AUDIO_FREQ_48K, LOOPBACK_MEM_SAM_CNT);
    LOG_INFO(LOOPBACK_AUDIO, "48kHzPlayRecPartDuration:%s;", FloatToStr(duration_s, 3) );
    return res;
}
...
bool loopback_audio_is_idle(uint8_t num) {
    ...
    if(Node) {
        bool p_res = wav_player_is_idle(Node->player_num);
        bool s_res = sound_recorder_is_idle(Node->rec_num);
        if(p_res ) {
            if(s_res) {
                res = true;
            }
...
bool loopback_audio_rec_file(const uint8_t num, const char* const RecFileName, const float rec_duration_s) {
    ...
    if(Node) {
        ...
        res = sound_recorder_set_temp_mem(Node->player_num, Node->RxSampleArray, Node->sample_array_size);
        res = sound_recorder_start(Node->rec_num, RecFileName, rec_duration_s);
        uint8_t i2s_num = loopback_audio_i2s_num_get(num);
        memset(Node->TxSampleArray, 0, sizeof(Node->sample_array_size) * sizeof(SampleType_t));
        res = i2s_mcal_write(i2s_num, (uint16_t*)Node->TxSampleArray, Node->sample_array_size);
...
bool loopback_audio_rec_test(const uint8_t num, const float rec_duration_s) {
    ...
    res = wav_file_name_generate(PlayFileName, sizeof(PlayFileName), LOOPBACK_MEM_SAM_CNT, rec_duration_s);
