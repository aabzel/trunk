...
static inline bool sound_rec_save_part_offset(SoundRecorderHandle_t* Node, uint32_t sam_index, uint32_t need_size) {
    bool res = false;
...
static inline bool sound_recorder_save_to_part2(SoundRecorderHandle_t* Node) {
    bool res = false;
    res = sound_rec_save_part_offset(Node, REC_MEM_HALF_SIZE, REC_HALF_SIZE_OF);
    return res;
}
                        ...
                        if(sizeof(WavHeader_t) == written) {
                            ...
                            if(res) {
                                ...
                                if(res) {
                                    Node->is_rec = true;
                                    Node->state = SOUND_RECORDER_STATE_REC_PART_1;
                                    led_mono_sw_pwm_duty(Node->led_num, 5);
                                }
                            }
                        }
                    }
