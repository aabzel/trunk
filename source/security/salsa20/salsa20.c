...
static void s20_expand16(uint8_t* k, uint8_t n[static 16], uint8_t keystream[static 64]) {
    int i, j;
    // The constants specified by the Salsa20 specification, 'tau'
    // "expand 16-byte k"
    uint8_t t[4][4] = {{'e', 'x', 'p', 'a'}, {'n', 'd', ' ', '1'}, {'6', '-', 'b', 'y'}, {'t', 'e', ' ', 'k'}};
...
static void s20_expand32(uint8_t* k, uint8_t n[static 16], uint8_t keystream[static 64]) {
    int i = 0, j = 0;
    // The constants specified by the Salsa20 specification, 'sigma'
    // "expand 32-byte k"
    uint8_t o[4][4] = {{'e', 'x', 'p', 'a'}, {'n', 'd', ' ', '3'}, {'2', '-', 'b', 'y'}, {'t', 'e', ' ', 'k'}};
...
s20_status_t s20_crypt(uint8_t* key, enum s20_keylen_t keylen, const uint8_t* const nonce, uint32_t si, uint8_t* buf,
                       uint32_t buflen) {
