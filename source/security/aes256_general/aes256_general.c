...
bool aes256_ecb_decrypt(const uint8_t* key, const uint8_t* const data_in, uint32_t datalen, uint8_t* const data_out,
                        uint32_t* returnlen) {
    bool res = false;
#ifndef HAS_AES256_V1
...
bool aes256_ecb_encrypt(const uint8_t* key, uint8_t* data_in, uint32_t datalen, uint8_t* data_out,
                        uint32_t* returnlen) {
    bool res = false;
#ifndef HAS_AES256_V1
    aes256_context_t ctx;
    ...
    for(b = 0; b < blk_cnt; b++) {
        ...
        if(AES_SUCCESS == ret) {
            LOG_DEBUG(AES256, "Ok %u/%u", b,blk_cnt);
            res = true;
        } else {
            LOG_ERROR(AES256, "Err");
            res = false;
...
bool aes256_cbc_encrypt(uint8_t* const key, uint8_t* const iv, uint8_t* const in_data, uint32_t datalen,
                        uint8_t* const out_data, uint32_t* const returnlen) {
    bool res = false;
#ifndef HAS_AES256_V1
    AES_ctx ctx;
...
bool aes256_cbc_decrypt(uint8_t* key, uint8_t* iv, uint8_t* in_data, uint32_t datalen, uint8_t* out_data,
                        uint32_t* returnlen) {
    bool res = false;
#ifndef HAS_AES256_V1
...
bool aes256_cbc_decrypt_stream(AES_ctx* ctx, uint8_t* key, uint8_t* in_data, uint32_t datalen, uint8_t* out_data,
                               uint32_t* returnlen) {
    bool res = false;
#ifndef HAS_AES256_V1
    LOG_DEBUG(AES256, "CBC DeCrypt %u byte", datalen);
