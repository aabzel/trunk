...
bool time_sec_to_time(uint32_t sec, struct tm* const time) {
    ...
    if(time) {
        time->tm_sec = sec % 60;
        time->tm_min = ((int)SEC_2_MIN(sec)) % 60;
        time->tm_hour = ((int)SEC_2_HOUR(sec)) % 24;
        time->tm_mday = ((int)SEC_2_DAYS(sec)) % 30;
        res = true;
    }
    return res;
}
...
bool time_delay_ms(uint32_t delay_in_ms) {
    bool res = false;
    uint32_t init_time_in_ms = time_get_ms32();
    uint32_t cur_ms = time_get_ms32();
    uint32_t cnt = 0;
    while(cur_ms < (init_time_in_ms + delay_in_ms)) {
        cur_ms = time_get_ms32();
        res = true;
        cnt++;
    }
#ifdef HAS_LOG
    LOG_DEBUG(TIME, "WaitDone Cnt:%u", cnt);
#endif
    return res;
}
#endif
...
#ifdef HAS_LOG
                LOG_ERROR(TIME, "SaveMaxUpTimeErr %u ms", up_time_ms);
#endif
            }
        }
#endif /*HAS_PARAM*/
