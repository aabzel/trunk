...
void HandleTag(void* cookie, const char* tag_name) {
    ...
    if(tag_name) {
        ...
        if(tag_name[0] != '?') {
            ...
            bool res = false;
#ifdef HAS_HASHSET
            res = hashset_push((HashSet_t* const) & XmlHashSet,( uint8_t *) tag_name, strlen(tag_name));
            if(false == res) {
                XmlItem.err_cnt++;
            }
            ...
            char* new_name = strdup((char *)node_name);
            if(new_name) {
                Array_t node = {
                    .size = 0,
                    .pArr = NULL,
                };
                node.pArr = (uint8_t* ) new_name;
...
void HandleTagEnd(void* cookie, const char* tag_end) {
    ...
    if(0 == ret) {
        if(KeePassItem.cur_entry < (KEEPASS_MAX_ENTRY - 1)) {
            KeePassItem.cur_entry++;
        } else {
            LOG_DEBUG(CRY, "EnrtyOvervlow: Cur:%u,Max:%u", KeePassItem.cur_entry, KEEPASS_MAX_ENTRY);
        }
    }
...
void HandleContent(void* cookie, const char* content) {
    ...
    if(is_string_node) {
        ...
        if(is_value_node) {
            if(KeePassItem.cur_entry < KEEPASS_MAX_ENTRY) {
                ...
                if(is_password_node) {
                    ...
                    if(strlen(content) < PASSWORD_SIZE) {
                        strncpy((char *)KeePassItem.entry[KeePassItem.cur_entry].PassWord.base64,
                                (char *)content,
                                (size_t)(PASSWORD_SIZE - 1));
                ...
                LOG_ERROR(XML, "EntryOverflow: Cur:%u,Max:%u", KeePassItem.cur_entry, KEEPASS_MAX_ENTRY);
            }
        }
    }
#endif
