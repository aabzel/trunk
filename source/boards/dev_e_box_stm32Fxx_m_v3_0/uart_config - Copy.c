#include "uart_config.h"

#include "log_config.h"
#include "data_utils.h"

/*constant compile-time known settings*/
const UartConfig_t UartConfigLuT[] = {
#ifdef HAS_LOG
    { .num=UART_NUM_LOG,   .baud_rate=LOG_UART_BAUD_RATE,   .name = "LOG",   .rx_buff_size=0, .valid=true,},
#endif
#ifndef HAS_BOOTLOADER
    { .num=6,   .baud_rate=LOG_UART_BAUD_RATE,   .name = "Test",   .rx_buff_size=0, .valid=true,},
#endif
};

uint8_t LogUartTxArray[UART_TX_FIFO_SIZE];
uint8_t Uart6TxArray[UART_TX_FIFO_SIZE/4];

UartHandle_t UartInstance[]={
#ifdef HAS_LOG
    {.num=UART_NUM_LOG, .valid=true, .TxFifo={.err_cnt=0,
                                              .initDone=true,
                                              .array=(char *)LogUartTxArray,
                                              .fifoState={
                                                          .size=sizeof(LogUartTxArray),
                                                          .start=0,
                                                          .end=0,
                                                          .count=0,
                                                          .errors=false,
                                              },
    },
    },
#endif
#ifndef HAS_BOOTLOADER
    {.num=6, .valid=true, .TxFifo={.err_cnt=0,
                                              .initDone=true,
                                              .array=(char *)Uart6TxArray,
                                              .fifoState={
                                                          .size=sizeof(Uart6TxArray),
                                                          .start=0,
                                                          .end=0,
                                                          .count=0,
                                                          .errors=false,
                                              },
    },
    },
#endif
};

uint32_t uart_get_cnt(void) {
    uint32_t cnt = 0;
    uint32_t cnt_conf = ARRAY_SIZE(UartConfigLuT);
    uint32_t cnt_ints = ARRAY_SIZE(UartInstance);
    if(cnt_conf==cnt_ints){
        cnt = cnt_ints;
    }
    return cnt;
}

