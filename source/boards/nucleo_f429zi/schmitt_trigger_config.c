...
static bool schmitt_trigger1_proc_up(void) {
    bool res = false;
    SchmittTriggerHandle_t* Node=SchmittTriggerGetNode(1);
    if(Node){
    	res = true;
        LOG_DEBUG(SCHMITT_TRIGGER, "1 Up, Val:%f",Node->value);
    }
    ...
    ret = le_audio_pause();
    if(ret) {
    	LOG_DEBUG(AUTO_VOLUME, "LeAudioPauseErr %d=%s", ret, ErrNo2Str(ret));
        res = true;
    } else {
        res = true;
...
static bool schmitt_trigger1_proc_down(void) {
    ...
    if( Node){
        res = true;
        LOG_DEBUG(SCHMITT_TRIGGER, "1 Down, Val:%f",Node->value);
    }
    ...
    if(false == AutoVolumeItem.on) {
        ...
        ret = le_audio_play();
        if(ret) {
        	LOG_DEBUG(AUTO_VOLUME, "LeAudioPlayErr %d=%s", ret, ErrNo2Str(ret));
            res = false;
        } else {
            res = true;
...
static bool schmitt_trigger2_proc_up(void) {
    bool res = true;
    LOG_DEBUG(SCHMITT_TRIGGER, "FF2 Up");
    return res;
...
static bool schmitt_trigger2_proc_down(void) {
    bool res = true;
    LOG_DEBUG(SCHMITT_TRIGGER, "FF2 Down");
    return res;
}
