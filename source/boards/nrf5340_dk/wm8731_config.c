...
const Wm8731RegConfig_t Wm8731RegConfigLut[]={
    ...
    {.reg_addr=0x02, .value={.LeftHeadOutCtrl.lhpvol=127,  .LeftHeadOutCtrl.lzcen=1,}},
    {.reg_addr=0x03, .value={.RightHeadOutCtrl.rhpvol=127, .RightHeadOutCtrl.rzcen=1,}},
    ...
    {.reg_addr=0x07, .value.DigitalAudioIfCtrl.format=FMT_I2S,
                     .value.DigitalAudioIfCtrl.iwl=AUD_BIT_16,//AUD_BIT_24 AUD_BIT_32
...
uint32_t wm8731_get_config_cnt(void){
    uint8_t cnt = 0;
    cnt = ARRAY_SIZE(Wm8731RegConfigLut);
    return cnt;
}
