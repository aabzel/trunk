...
#endif /*HAS_HASHSET*/
        snprintf(node_name, sizeof(node_name), "%s_%u", tag_name, (unsigned int)cnt);
#ifdef HAS_XML_2_DOT
        fprintf(dot_file, "%s [label=%s] [shape=box]" CRLF, node_name, tag_name);
        Array_t ParentNode = {0};
        res = lifo_arr_peek(&XmlLifoArray, &ParentNode);
        if(res) {
            fprintf(dot_file, "%s->%s" CRLF, ParentNode.pArr, node_name);
        }
#endif /*HAS_XML_2_DOT*/
...
void HandleParameter(void* cookie, const char* parameter) {
    ...
    res = lifo_arr_peek(&XmlLifoArray, &ParentNode);
    if(res) {
        fprintf(dot_file, "%s->%s" CRLF, ParentNode.pArr, parameter);
    }
#endif
}
...
void HandleContent(void* cookie, const char* content) {
    ...
    res = lifo_arr_peek(&XmlLifoArray, &ParentNode);
    if(res) {
        fprintf(dot_file, "%s->cont_%u" CRLF, ParentNode.pArr, cont_cnt);
    }
#endif
...
void HandleAttribute(void* cookie, const char* attribute) {
    ...
    res = lifo_arr_peek(&XmlLifoArray, &ParentNode);
    if(res) {
        fprintf(dot_file, "%s->attr_%u %s", ParentNode.pArr, attr_cnt);
    }
#endif
}
