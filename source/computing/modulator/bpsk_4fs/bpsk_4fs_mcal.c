...
static uint32_t bpsk_4fs_encode_chirp(Bpsk4fsHandle_t* Node, uint32_t s, const uint8_t* const data, uint32_t bit,
    ...
    LOG_DEBUG(BPSK_4FS, "Data:0%x,ByteN:%u,BitN:%u,BitVal:%u,samPerChip:%u Sam" CRLF, data[byte_num], byte_num, bit_num,
              bit_val, Node->sample_per_chip);
...
static bool bpsk_4fs_encode_ll(Bpsk4fsHandle_t* Node, const uint8_t* const data, uint32_t data_size,
...
bool bpsk_4fs_encode(uint8_t num, const uint8_t* const message, const uint32_t message_size, int32_t* const sample,
                     uint32_t sample_cnt, uint32_t* const sample_len) {
    bool res = false;
...
#ifdef HAS_WAV
static bool bpsk_4fs_compose_wav_header_ll(const Bpsk4fsHandle_t* const Node, WavHeader_t* const Header,
...
#ifdef HAS_WAV
bool bpsk_4fs_encode_to_wav(uint8_t num, const uint8_t* const data_to_encode, const uint32_t data_size,
...
static bool bpsk_4fs_decode_decimator(Bpsk4fsHandle_t* const Node) {
    ...
    if(spot_bit) {
...
#ifdef HAS_MANCHESTER_DECODE
        uint8_t  m_out = 0xFF;
        res = manchester_decode_proc_sample(1, ( uint8_t) bit_data, &m_out);
#endif
        ...
        Node->rx_byte= bit_u8_ctrl( Node->rx_byte,  (7 - bitNum), bit_data);
 ...
 */
static bool bpsk_4fs_decode_ll(Bpsk4fsHandle_t* const Node, const int32_t* const SignalArray, uint32_t sample_cnt,
                               uint8_t* const raw_data, uint32_t const raw_data_size, uint32_t* const data_size) {
    bool res = false;
    if(Node) {
        if(SignalArray) {
            if(sample_cnt) {
...
/*
 */
bool bpsk_4fs_decode(uint8_t num, const int32_t* const SampleArray, uint32_t sample_cnt, uint8_t* const raw_data,
                     const uint32_t raw_data_size, uint32_t* message_size) {
    bool res = false;
    Bpsk4fsHandle_t* Node = Bpsk4fsGetNode(num);
    if(Node) {
...
static bool bpsk_4fs_init_decoder_common(const Bpsk4fsConfig_t* const Config, Bpsk4fsHandle_t* const Node) {
    bool res = true;
...
static bool bpsk_4fs_proc_from_adc_mem(Bpsk4fsHandle_t* Bpsk) {
    ...
    if(Adc) {
        ...
        if(Adc->half_cplt_done) {
            ...
            res = proc_samples_to_mem(Bpsk, Adc->RxSamples, 0, Adc->RxSamplesCnt / 2);
        ...
        if(Adc->chan_conv_end) {
            ...
            res = proc_samples_to_mem(Bpsk, Adc->RxSamples, Adc->RxSamplesCnt / 2, Adc->RxSamplesCnt);
