...
uint32_t calc_crc24_q(const uint8_t* const data, uint32_t const len) {
    uint32_t crc = 0;
    uint32_t i = 0;
    for(i = 0; i < len; i++) {
        crc = (crc << 8) ^ Crc24qLut[data[i] ^ (uint8_t)(crc >> 16)];
    }
    crc = (MASK_24BIT & crc);
    return crc;
}
...
bool crc24_q_check(const uint8_t* const in_data, uint32_t const length, uint32_t const crc24_read) {
    bool res = false;
    uint32_t crc24_calc = 0x0000;
    crc24_calc = calc_crc24_q(in_data, length);
    if(crc24_calc == crc24_read) {
        res = true;
    } else {
#ifdef HAS_LOG
        LOG_DEBUG(CRC24, "Err Calc:0x%08x Read:0x%08x", crc24_calc, crc24_read);
#endif
    }
    return res;
}
