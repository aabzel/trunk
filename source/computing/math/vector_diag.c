...
const char* VectorToStr(const VectorF_t* const Node) {
    if(Node) {
        strcpy(text, "");
...
void print_vector(VectorF_t v, char* in_text) { LOG_INFO(MATH, "%s%s", in_text, VectorToStr(&v)); }
        ...
        for(V1.dy = -VEC_LIM; V1.dy <= VEC_LIM; V1.dy += VEC_STEP) {
            ...
            for(V2.dx = -VEC_LIM; V2.dx <= VEC_LIM; V2.dx += VEC_STEP) {
                for(V2.dy = -VEC_LIM; V2.dy <= VEC_LIM; V2.dy += VEC_STEP) {
                    float angle_deg = calc_angle_between_vectors_deg(&V1, &V2);

#if 0
                    float angle_naiv_deg = calc_fll_discriminator_deg(&V1, &V2);
                    res = is_float_equal_absolute(angle_naiv_deg, angle_deg, 0.001);
#endif
