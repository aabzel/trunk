...
bool try_strl2double(const char double_str[], int32_t str_len, double* double_value) {
    ...
    if(double_str) {
        ...
        memcpy(tempStr, double_str, (size_t)str_len);
...
#ifndef STM32F4xx
    uint8_t code = (uint8_t)__fpclassifyf(float_data_);
    switch(code) {
    case FP_NORMAL:
    case FP_ZERO:
    case FP_SUBNORMAL:
...
bool try_str2array(char* in_str_array, uint8_t* out_array, uint16_t array_size, size_t* const out_array_len) {
    bool res = false;
    uint8_t out_shift = 0;
...
bool try_str2number(const char* const text, double* const double_value) {
    bool res = false;
    uint32_t len = 0;
    if(text) {
        len = strlen(text);
    ...
    if(res) {
        ...
        uint32_t ok = 0;
        uint32_t err = 0;
        LOG_DEBUG(LINE, "ParseDoubleIn:%u:[%s]", len, text);
        ...
        for(i = 0; i < len; i++) {
            ...
            if(res) {
                ...
                err++;
                LOG_ERROR(LINE, "ProcErr: ch[%u]=%c ", i, text[i]);
                break;
            }
        }
        ...
        if(0 == err) {
            *double_value = Fsm.value;
            ...
            LOG_ERROR(LINE, "err:%u,len:%u,ok:%u", err, len, ok);
            res = false;
        }
    }
    return res;
 ...
 * */
bool try_str2type(const char* const text, StorageType_t type, uint8_t* out_buff, size_t buff_size) {
    bool res = false;
    LOG_DEBUG(LINE, "Parse:%s In[%s]OutSize:%u byte", text, StorageTypeToStr(type), buff_size);
    if(text) {
        if(out_buff) {
            if(buff_size) {
                switch(type) {
...
bool try_str2data_type(const char* const text, StorageType_t* const type) {
    bool res = false;
    if(text) {
        if(type) {
        }
    ...
    int8_t s8_value = 0;
    res = try_str2int8(text, &s8_value);
    if(res) {
        *type = TYPE_INT8;
    }

    uint8_t u8_value = 0;
    res = try_str2uint8(text, &u8_value);
    if(res) {
        *type = TYPE_UINT8;
    }
    return res;
