...
const char* str_del_char_inplace(char* const data, char del_char) {
    ...
    if(data) {
        ...
        for(i = (lenght - 1); 0 <= i; i--) {
            if(data[i] == del_char) {
                res = array_u8_del_index((uint8_t*)data, lenght, (size_t)i);
                if(res) {
                    del_cnt++;
                }
            }
    ...
    if(data) {
        ...
        if(substr) {
            ...
            for(i = (lenght - 1); 0 <= i; i--) {
                ...
                if(0 == ret) {
                    res = array_u8_del_indices((uint8_t*)data, lenght, (size_t)i, sub_len);
                    if(res) {
                        del_cnt++;
                    }
                }
...
char* str_limit(char* str, uint32_t size) {
    static char temp_str[150] = "";
    uint32_t len = strlen(str);
    if(len < sizeof(temp_str)) {
        strcpy(temp_str, str);
    }
    if((size + 3) < sizeof(temp_str)) {
        if(size < len) {
            temp_str[size] = '.';
            temp_str[size + 1] = '.';
            temp_str[size + 2] = '\0';
...
uint32_t str_delete_first(char* text, size_t size, char letter) {
    uint32_t i = 0;
    uint32_t cnt = 0;
    LOG_DEBUG(LINE, "DelFront:[%s]%u,Ch:[%c]", text, size, letter);
    bool res = false;
    for(i = 0; i < size; i++) {
        if(letter == text[0]) {
            res = array_u8_del_index((uint8_t*)text, size, 0);
            if(res) {
                cnt++;
            }
        } else {
...
uint32_t string_delete_end(char* const text, size_t size, char letter) {
    ...
    if(text) {
        if(size) {
            LOG_DEBUG(LINE, "DelEnd:[%s]%u,Ch:[%c]", text, size, letter);
            int32_t i = 0;
            for(i = (size - 1); 0 <= i; i--) {
                LOG_DEBUG(LINE, "Proc:%u:[%c]", i, text[i]);
                if(letter == text[i]) {
                    text[i] = 0x00;
...
uint32_t string_delete_continuous_duplicat(char* const text, char letter) {
    ...
    LOG_DEBUG(LINE, "DelDup:[%s]:%u, Del[%c]", text, size, letter);
    uint32_t i = 0;
    uint32_t j = 0;
    bool res = false;
    for(j = 0; j < size; j++) {
        ...
        for(i = 0; i < size; i++) {
            ...
            if(letter == cur) {
                if(cur == prev) {
                    if(0 < i) {
                        res = array_u8_del_index((uint8_t*)text, size, i);
                        if(res) {
                            cnt++;
                        }
                        break;
