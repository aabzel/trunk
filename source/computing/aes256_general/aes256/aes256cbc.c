...
void AES256CBC_init_ctx_iv(AES_ctx* ctx, const unsigned char* key, const unsigned char* iv){
	LOG_INFO(AES256,"CrcInit");
	if(ctx && key && iv){
        KeyExpansion(ctx->RoundKey, key);
        if(ctx->Iv){
            LOG_INFO(AES256,"CopyVi");
            memcpy(ctx->Iv, iv, AES_BLOCKLEN);
        }else{
        	LOG_ERROR(AES256,"IvErr");
        }
	}else{
		LOG_ERROR(AES256,"ArgErr");
	}
}
...
void AES256CBC_encrypt(AES_ctx *ctx, unsigned char* buf, unsigned  int length)
{
    unsigned  int i;
...
void AES256CBC_decrypt(AES_ctx* ctx, unsigned char* buf,  unsigned  int length){
    if(buf && length){
       	LOG_DEBUG(AES256,"AES256CBC_decrypt Len:%u",length);
        unsigned  int i;
        unsigned char storeNextIv[AES_BLOCKLEN];
        for (i = 0; i < length; i += AES_BLOCKLEN)    {
        	LOG_DEBUG(AES256,"CBC De i:%u",i);
