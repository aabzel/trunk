...
void AES256MainECB(const uint8_t* key, uint8_t* data, uint32_t datalen, uint8_t* returndata, uint32_t* returnlen,
                   bool encrypt) {
    LOG_DEBUG(AES, "%s()", __FUNCTION__);
    int numbBlocks = ceil(datalen / 16.0);
    LOG_DEBUG(AES, "NumBlock:%u", numbBlocks);
    int dataBytes = 0;
    int returnBytes = 0;
    int32_t i, x, y;
...
bool AES256MainCBC(uint8_t* key, uint8_t* iv, uint8_t* data, uint32_t datalen, uint8_t* returndata, uint32_t* returnlen,
                   bool encrypt) {
    bool res = false;
    LOG_INFO(AES, "AES256MainCBC() Encrypt %u", encrypt);
    if(key && iv && data && returndata && returnlen) {
        res = true;
        int numbBlocks = ceil(datalen / 16.0);
        int dataBytes = 0;
        int returnBytes = 0;
        ...
        *returnlen = returnBytes;
        ...
        LOG_ERROR(AES, "ArgErr");
    }
    return res;
}

void AES256MainCTR(uint8_t* key, uint8_t* data, uint8_t* iv, uint32_t datalen, uint8_t* returndata, uint32_t* returnlen,
                   bool encrypt) {
    LOG_INFO(AES, "%s()", __FUNCTION__);
