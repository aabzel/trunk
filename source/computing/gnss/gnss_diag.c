...
bool print_wgs84_coordinate(Wgs84Coordinate_t Wgs84Coordinate, bool one_line) {
    ...
    cli_printf("XYZ %f %f %f m Acc:%f m" CRLF, Wgs84Coordinate.x_cm / 100.0, Wgs84Coordinate.y_cm / 100.0,
               Wgs84Coordinate.z_cm / 100.0, Wgs84Coordinate.acc_cm / 100.0);
...
const char* GnssCoordinateToStr(const GnssCoordinate_t* const Node) {
    static char locStr[100] = "";
    if(Node) {
        bool res = is_valid_gnss_coordinates(Node);
        if(res) {
            snprintf(locStr, sizeof(locStr), "%7.6f,%7.6f", Node->phi, Node->lambda);
        } else {
            strcpy(locStr, "Inval");
        }
    }
...
const char* GnssCoordinateDataToStr(const void* const data) { return GnssCoordinateToStr((GnssCoordinate_t*)data); }
