...
static volatile float SECTION_CCMRAM phaseErrRadLUT_iqPos[PHA_ERR_LUT_LEN][PHA_ERR_LUT_LEN]={0};
static volatile float SECTION_CCMRAM phaseErrRadLUT_iPosQNeg[PHA_ERR_LUT_LEN][PHA_ERR_LUT_LEN]={0};
static volatile float SECTION_CCMRAM phaseErrRadLUT_iNegQpos[PHA_ERR_LUT_LEN][PHA_ERR_LUT_LEN]={0};
static volatile float SECTION_CCMRAM phaseErrRadLUT_iNegQneg[PHA_ERR_LUT_LEN][PHA_ERR_LUT_LEN]={0};
...
static inline bool quad_mix_4fs_proc_sample_i(QuadMix4fsHandle_t* const Node, int32_t sam_num,
        int32_t sample) {
    bool res = false;
...
static inline bool quad_mix_4fs_proc_sample_q(QuadMix4fsHandle_t* const Node, int32_t sam_num,
        int32_t sample) {
    bool res = false;
...
inline float quad_mix_4fs_calc_phase_error_atan(QuadMix4fsHandle_t* const Node) {
    ...
    if(res) {
        phase_error_rad = calc_carrier_phase_err_rad((float) Node->SdrI.AfterFilt,
                                                     (float) Node->SdrQ.AfterFilt);
    }
...
bool quad_mix_4fs_constellation_diagram(QuadMix4fsHandle_t* const Node) {
    bool res = false;
    uint16_t code;
    code = DAC_12BIT_HALF + 2 * Node->SdrI.AfterFilt;
    res = dac_channel_code_set(1, DAC_CHAN_1, code);
    code = DAC_12BIT_HALF + 2 * Node->SdrQ.AfterFilt;
...
inline bool quad_mix_4fs_proc_sample_ll(QuadMix4fsHandle_t* const Node,
                                        int32_t sam_num, int32_t sample) {
    bool res = false;
...
static bool quad_mix_4fs_init_common(const QuadMix4fsConfig_t* const Config,
                                     QuadMix4fsHandle_t* const Node) {
    bool res = false;
    if (Config) {
        if (Node) {
            Node->num = Config->num;
