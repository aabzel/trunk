...
bool correlator_s16_expract_mem(uint8_t num, uint32_t need, double* const value) {
    bool res = false;
    LOG_DEBUG(CORRELATOR_S16, "Need %u", need);
    ...
    if(Node) {
        ...
        if(value) {
            LOG_DEBUG(CORRELATOR_S16, "Need: %u Phy:%u Val:%f", need, ind, Node->x[ind]);
            *value = Node->x[ind];
            res = true;
        }
    }
...
bool CorrelatorS16DiagConfig(const CorrelatorS16Config_t* const Config) {
    bool res = false;
    if(Config) {
        LOG_INFO(CORRELATOR_S16, "N:%u,Order:%u [%s]", Config->num, Config->size, Config->name);
        res = true;
    }
    return res;
}
...
#ifdef HAS_PC
#define CORRELATOR_S16_STRUCTURE_FILE "CorrelatorS16Structure.gv"
...
#ifdef HAS_PC
bool correlator_s16_node_generate_graphviz(CorrelatorS16Handle_t* Node, FILE* FilePtr, int x, int y, char* lText,
                                           double val) {
    bool res = false;
...
bool correlator_s16_generate_graphviz(uint8_t num) {
    ...
    if(Node) {
        int16_t in = 0.0;
        int64_t out = 0.0;
        res = correlator_s16_pop_ll(Node, in, &out);
        int16_t mult = 0;
        FILE* GvFilePtr = NULL;

        char GvFileName[80] = "";
        sprintf(GvFileName, "CorrelatorS16%u_Code.gv", num);

        GvFilePtr = fopen(GvFileName, "w");
        if(GvFilePtr) {
            // int ret = 0;
            fprintf(GvFilePtr, "digraph CorrelatorS16Filter {\n");
            ...
            sprintf(SvgFileName, "CorrelatorS16_%u_Schematic.svg", num);
            ...
            LOG_ERROR(CORRELATOR_S16, "FileGenerateErr");
        }
    } else {
...
bool correlator_s16_diag(uint8_t num) {
    ...
    if(Node) {
        ...
        for(i = 0; i < Node->size - 1; i++) {
            ...
            xi = 0.0;
            correlator_s16_expract_mem(num, i, &xi);
...
bool correlator_s16_diag_mem(uint8_t num) {
    ...
    CorrelatorS16Handle_t* Node = CorrelatorS16GetNode(num);
    if(Node) {
        LOG_INFO(CORRELATOR_S16, "CORRELATOR_S16%u,ProcCnt:%u", num, Node->proc_cnt);
        circular_buffer_index_diag(&Node->indexer);
        ...
        for(i = 0; i < Node->size - 1; i++) {
            ...
            double x = 0.0;
            correlator_s16_expract_mem(num, i, &x);
