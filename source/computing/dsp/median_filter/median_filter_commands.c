...
bool median_filter_calc_frequency_response_command(int32_t argc, char* argv[]) {
    ...
    if(res) {
        res = median_filter_calc_frequency_response(num);
    } else {
        LOG_ERROR(MEDIAN_FILTER, "Usage: iifr num");
    }
    return res;
}
...
bool median_filter_calc_frequency_response_norm_command(int32_t argc, char* argv[]) {
    ...
    if(res) {
        res = median_filter_calc_frequency_response_norm(num);
    } else {
        LOG_ERROR(MEDIAN_FILTER, "Usage: iifrn num");
    }
    return res;
}
#endif
...
bool median_filter_diag_command(int32_t argc, char* argv[]) {
    ...
    if(res) {
        res = median_filter_diag();
    } else {
        LOG_ERROR(MEDIAN_FILTER, "Usage: median_filter");
    }
    return res;
}
...
bool median_filter_proc_command(int32_t argc, char* argv[]) {
    ...
    if(res) {
        ...
        LOG_ERROR(MEDIAN_FILTER, "Usage: iip In");
    }
    return res;
}
