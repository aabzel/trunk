...
static bool scan_init_custom(void) {
    return true;
}
...
bool scan_proc_val(uint8_t num, float value_in, ScanAction_t* const action) {
    ...
    if(Node) {
        if(action) {
            ...
            switch (Node->state) {
            case SCAN_STATE_ACC: {
                res = scan_proc_acc_ll(Node);
            }
                break;
            case SCAN_STATE_CAPTURE: {
                res = scan_proc_capture_ll(Node);
            }
                break;
            case SCAN_STATE_UNDEF:
                Node->state = SCAN_STATE_ACC;
                res = true;
                break;
            default:
