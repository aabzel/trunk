...
ifneq ($(SOLVER_SLAE_MK_INC),Y)
    ...
    # $(error SOLVER_SLAE_DIR= $(SOLVER_SLAE_DIR))
    INCDIR += -I$(SOLVER_SLAE_DIR)

    MCAL_OPT += -DHAS_SOLVER_SLAE
    SOURCES_C += $(SOLVER_SLAE_DIR)/solver_slae.c

    ifeq ($(SOLVER_SLAE_DIAG),Y)
        # $(error SOLVER_SLAE_DIAG= $(SOLVER_SLAE_DIAG))
        MCAL_OPT += -DHAS_SOLVER_SLAE_DIAG
        SOURCES_C += $(SOLVER_SLAE_DIR)/solver_slae_diag.c
    endif

    ifeq ($(SOLVER_SLAE_COMMANDS),Y)
        # $(error SOLVER_SLAE_COMMANDS= $(SOLVER_SLAE_COMMANDS))
        MCAL_OPT += -DHAS_SOLVER_SLAE_COMMANDS
